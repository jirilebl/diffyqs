...
a solution \myquote{locally}---only for a short period of time or
...
A diagram of a pendulum.  A pendulum is shown fixed at a point at the top
moving along a circular arc.  The bottom of the pendulum is labeled m, and
the angle the pendulum makes with a vertical line is labeled theta.
The length of the pendulum is labeled L.}
\end{mywrapfigsimp}
In \sectionref{sec:mv} we looked at the pendulum of
...
not even be interested in the actual solution.
We might only be interested
...
A direction field plot (a grid of arrows) in the xy-plane.
In the center, the arrows go around the critical point at the
origin in a clockwise direction.  Then
at the point (1,0) the direction field is a saddle point, that is,
the arrows along a diagonally down sloped curve go towards the critical point
and in a diagonally upwards sloped curve go away from the critical point.
To the left of the critical point, the arrows go roughly down, above the critical
point the arrows go roughly rightwards,
on the right of the critical point the arrows point roughly upwards,
and below the critical point the arrows point roughly leftwards.
The arrows appear longer the further away from the critical point they are.
Several trajectories are also drawn.  Around the critical point at
(0,0) there are several egg-shaped trajectories with the pointy end in the
positive x-direction.  Then there is one trajectory seeming to
go 'through' the critical point at (1,0) making a loop, although if one
were to make a loop, one would have to go against the arrows once one goes
through the critical point.  Then there are several other trajectories,
there are several trajectories that start on the bottom right go diagonally
up but then turn leftwards until they get to the left half of the picture
staying below the x-axis.
Then they turn upwards until we
are in the upper half of the picture and then turn rightwards until they are
in the right half of the picture where they turn diagonally upwards leaving
the picture.  Then a couple of other trajectories start below the x-axis on
the right side of the picture close to the x-axis, then simply turn
rightwards until they leave the picture, never making it past x equals 1.}
...
\end{bmatrix} ,
\end{equation*}
and our new variables are
$u=x$ and $v=y$.  The linearization at $(0,0)$ is
\begin{equation*}
\begin{bmatrix} u \\ v \end{bmatrix} ' =
\begin{bmatrix}
0 & 1 \\
...
critical points $(0,0)$ and $(1,0)$ are given in \figurevref{fig:nlin-1b-lin}.  Note
that the variables are now $u$ and $v$, which are different at each
critical point.  Compare
...
Two direction field plots (grid of arrows) in the uv-plane.
On the left the arrows simply go clockwise around the origin, with several
circular trajectories drawn.  On the right is a
picture of a saddle where along the line v equals u the arrows point away from the
origin and along v equals minus u the arrows point towards the origin.  The
lines v equals u
and v equals minus u are drawn as trajectories.  In the picture divided by these lines,
in the region on the left, the arrows point generally downward, towards the
top going slightly to the right and towards the bottom going slightly to the
left.  There is a trajectory in this region
starting out of the picture on the left side
going down and slightly to the right before turning to the left
and leaving the picture.  The situation is analogous in the other 3 regions,
where in the top region the arrows go roughly rightwards, the arrows in the
right region go roughly upwards, and the arrows in the bottom region go
roughly leftwards.}
...
A direction field in the xy-plane where all the arrows 
seem small and pointing directly upwards along the x-axis.  The arrows above the
x-axis point upwards and away from the origin and the arrows below
the x axis point also roughly upwards and towards the origin.}}
\task
\parbox[c]{1.75in}{\diffyincludegraphics{width=1.75in}{width=1.75in}{nlin-exer-x2-y2}{%
A direction field in the xy-plane where the arrow is zero at the origin,
on the x-axis point rightward, on the y-axis point upward.  In the first
quadrant, the arrows point roughly away from the origin, and in the third
quadrant they point roughly towards the origin.  In the second quadrant,
the arrows start rightwards at the right and slowly turn upwards as we move
right.  In the fourth quadrant, the arrows start upwards at the bottom and
turn rightwards as we move up.}}
\task
\parbox[c]{1.75in}{\diffyincludegraphics{width=1.75in}{width=1.75in}{nlin-exer-sinpiy-x}{%
A direction field in the xy-plane where the arrows seem to go around
counterclockwise around two points on the y-axis, one above the origin and
...
A direction field plot in the xy-plane.
On the left of the plot the arrows seem to go clockwise around a point
to the left of the origin on the x-axis.  Then on the right the arrows
seem to go towards a point to the right of the origin along a diagonal line
downwards, and away from that point along a diagonal line upwards.
On the left side of the plot the arrows are roughly upward,
on the top, they are roughly rightwards, on the right, they are
roughly upward, and on the bottom, they are roughly leftward.}}
\task
\parbox[c]{1.75in}{\diffyincludegraphics{width=1.75in}{width=1.75in}{nlin-exer-ypx2-mx}{%
A direction field plot in the xy-plane.
The arrows seem to be going around the origin clockwise, but not
in a perfectly circular way.
At the top, the arrows are mostly rightward although being somewhat upward
facing on the left and downward facing on the right and they are slightly
smaller in the middle of the picture.  On the left side
of the picture, the arrows are all pointing roughly leftward and upward,
although they are smaller on the bottom of the picture.  On the right
side of the picture, the arrows are pointing roughly rightward and slightly
downward being smaller on the bottom of the picture.  Along the bottom
of the picture from left to right, the arrows start upward and to the left
then slowly turn rightward, than downward until they end pointing
down and to the right at the bottom right corner.}} 
\task
\parbox[c]{1.75in}{\diffyincludegraphics{width=1.75in}{width=1.75in}{nlin-exer-y2-mx2}{%
A direction field plot in the xy-plane.
The arrows seem to come from the top left and point down and to the right
roughly.  They keep going in that direction but almost as if they want to
take a detour around the origin.
Those that start above the y equals minus x
line go in a curve around and above the origin, while those that
start below the line take a curve around and below the origin.}}
\end{tasks}
\end{exercise}
\exsol{%
...
That is, if we zoom in enough, it is the
only critical point we see.
In \exampleref{example:nlin-1b-example} above,
both critical points are isolated.
If, on the other hand, there were a whole curve of critical
...
linear.
An even worse example is the system $x' = x$, $y' = x^2$, which does not have 
...
that critical point, and we have computed the
...
A direction field plot in the xy-plane for x between minus 2 and 2
and y between minus 2 and 2.  There are two critical points, one at the
origin and one at (1, minus 1).  The origin is a saddle with arrows 
coming into it along an upwards diagonal curve and arrows going
out along the downwards diagonal curve.  The point (1, minus 1) is a sink with
arrows pointing towards it.  Several trajectories are shown with
trajectories that start on the right side for y roughly below
y equals 1 and
on the bottom roughly to the right of x equals minus 1 going into the sink.  Other
trajectories turn toward and leave the picture on the upper left side
of the plot.}
...
$\left[ \begin{smallmatrix} 0 & -1 \\ -1 & 0 \end{smallmatrix} \right]$
whose eigenvalues are $1$ and $-1$.  As the matrix is invertible, the system is almost linear
...
As both eigenvalues are real and negative, the critical
...
Consider
$x'=y+y^2e^x$,
$y'=x$.  First, we find the critical points.  These are the points where
...
A direction field plot in the xy-plane for x between minus 2 and 2
and y between minus 2 and 2.  The origin is a saddle critical point
with arrows coming into the origin along a downwards sloped curve
and arrows coming out along an upwards sloped curve.  The
point (0, minus 1) is a spiral source with arrows winding counterclockwise out
of the critical point.  Several trajectories are shown notably two
that spiral out of the critical point at (0, minus 1).}
\caption{The phase portrait with few sample trajectories of 
...
\frac{\sqrt{4-9y^4}}{2} .
\end{equation*}
At any point where $y \neq 0$ (so at most points near the origin), the eigenvalues have a positive real part ($y^2$ can
...
A direction field plot in the xy-plane.
The arrows around the origin seem to circle the origin in the
counterclockwise direction.  There is a very tightly wound trajectory shown
that winds out of the critical point and eventually leaves the picture (even
turning in a different direction from the winding).}
...
there can be no spiral sources or sinks.
The Jacobian matrix is
\begin{equation*}
\begin{bmatrix}
0 & 1 \\
...
In the example $x'=y$, $y'=y^3-x$,
show that for any trajectory, the distance
from the origin is an increasing function.
Conclude
that at the origin the system behaves like a spiral source.
Hint: Consider $f(t) =
{\bigl(x(t)\bigr)}^2 + 
...
trajectories as $t$ goes to infinity?
\end{exercise}
...
almost linear if in addition $f'(x_0) \neq 0$.  Figure out if the critical point is stable or unstable
...
We change the equation to a two-dimensional
...
critical points: either stable centers or saddle points.
...
\end{equation*}
The eigenvalues
of this matrix are $\lambda = \pm \sqrt{-\frac{g}{L}\cos \theta}$.
The
...
A direction field plot in the theta omega plane
(the angle theta being the horizontal direction)
for theta between minus 7 and 7 and for omega between minus 3 and 3.
There are 5 critical points shown.
There are 3 centers shown, one at the origin, one at (minus 2 pi,0)
and one at (2 pi,0).  The arrows are winding clockwise around the centers.
There are 2 saddles shown, one at (minus pi,0) and one at (pi,0).
Several trajectories are drawn.  Trajectories around the centers simply go
around in ellipses.  Trajectories that start high on the
left-hand side simply go in a wavy motion as if repelled by the centers along the
top of the plot and leave on the right-hand side of the plot.
The trajectories on the bottom do the same thing in reverse.
There are also the trajectories that go around the centers and hit the
saddles diagonally.}
...
$\theta = \pi$.  Here the pendulum is upside down.
Sure, you can balance the
...
\capstart
\inputpdft{nlin-pend}{%
Four diagrams of the possible motions of the pendulum.
From left to right, the first picture is a stationary pendulum hanging
down with the label 'theta equals 0'.
The second picture labeled 'Small angular velocities' is a
swinging pendulum with an arrow indicating back and forth motion in
a circular arc.   The third picture labeled 'theta equals pi' is a
stationary pendulum balanced exactly upwards.  The
final picture, labeled 'Large angular velocities', is a pendulum rotating
...
says how far our approximation is from the real period percentage-wise.
Note that $T_{\text{linear}}$ is simply a constant.
It does not change with the initial angle $\theta_0$.
The actual period $T$ gets larger and larger as
$\theta_0$ gets larger.
...
Two plots.  On the left plot, there is a plot of T and T sub linear as
functions of theta sub 0 for theta sub 0 between 0 and pi over 2.
The plot of T sub linear is a horizontal line at 2 pi,
while the plot of T starts at 2 pi on the left, but slowly rises to
well above 7 on the right.  On the right plot is a plot of the relative
error of the two quantities starting out at zero for theta sub 0 equals 0,
then rising and at about 0.15 on the right.}
...
One of the most common simple applications of nonlinear systems is the
...
(1860--1940).}
systems.  For example, such a system arises
...
different species.  Before the application of differential
equations, these periodic variations in the population baffled biologists.

We keep
with the classical example of hares and foxes in a forest.
It is the
...
On the left there is a direction field for x between 0 and 200 and y between
0 and 100.  The arrows rotate counterclockwise around the critical point
at (100,40).  Several trajectories are plotted, they are not quite circular,
they go roughly rightwards along the bottom, then turn to go back up roughly
diagonally and then go back down along the left-hand side.  The large
trajectories look like a very rounded triangle.
On the right is a plot for t between 0 and 40, where the graph of predators
is in thick red and prey in thin blue.  The predators start lower than prey.
The prey grow quickly at first reaching a peak when the predators are
growing, and then the prey goes down significantly.  Once the prey is
roughly
halfway down its peak, the predators peak and start going down.  The prey
stops falling and recovers and starts growing again, while predators keep
falling a little longer. Then the cycle repeats.
The prey curves go much higher peaking above 225,
while the predators peak somewhere around 80.  The minimum of the predators
is around 15, while the minimum of the prey is around 32.
The curve for the predators
is steeper going up and less steep going down, while the prey curve rises
and falls almost at the same steepness, perhaps rising a little slower.}
...
\end{split}
\end{equation*}
So $C$ is constant along the trajectories.  In fact, the expression $C =
...
measurements, we know the Earth will not fly out into empty space
...
angular velocity is $\omega_0$.  Find an integral expression for how long it takes
...
\begin{exercise}
Suppose we have the predator-prey system where the foxes are also
...
The constant of motion is $C = y^ae^{-cx-by}$.
For any $C$, this curve must hit the $y$-axis (why?),
so the trajectory will simply approach a point on the $y$
...
For simplicity, we use $\mu = 1$.  A
phase diagram is given in the left-hand plot in
\figurevref{fig:nlin-van-der-fig}, where $y=x'$ as usual.  Notice how the
...
On the left is a phase plane (direction plot) of a Van der Pol oscillator where
the horizontal axis is the x and the vertical axis is the derivative x
prime.  The horizontal axis is between minus 4 and 4, and so is the
vertical.  On this plot the arrows seem to be going around the origin
in a clockwise manner.
However, there is a single trajectory that is a closed curve (a loop),
which is called the limit cycle.  Its shape is very roughly a sideways,
rounded rectangle.  It is roughly between minus 2 and 2 on the
horizontal x axis and between minus 3 and 3 on the vertical axis.
A trajectory starting inside this
limit cycle spirals outwards towards the limit cycle and gets closer
and closer to it.  Similarly, several trajectories starting outside approach
the limit cycle from the outside and get closer and closer to it.  Both from
the inside and the outside, the approach is very quick and the trajectories
do not even go around once before being indistinguishable from the limit
cycle.  On the right there is a plot on the tx-plane for t between 0 to 30
of a sample
solution starting near the origin.  It quickly goes into
a large periodic oscillation between roughly minus 2 and 2.}
...
example of \emph{\myindex{relaxation oscillation}}.  The word
relaxation refers to the sudden jump (the very steep part of the solution).
For larger $\mu$, the steep part is more pronounced.
For smaller $\mu$,
the limit cycle looks more like a circle.  In fact, if
$\mu = 0$, then $x''+x=0$, which is a linear system with a
center and all trajectories are circles.
\end{example}
...
Given a closed trajectory in an autonomous system,
...
and \eqref{nlin:gensys} has no critical points in $R$.
...
The point of the theorem is that if you find one solution that exists
for all $t$ large enough (that is, as $t$ goes to infinity) and stays
within a bounded region without critical points, then
you have found either a periodic orbit or a solution that spirals towards a
limit cycle---in the long term, the
solution is very close to a periodic function.
If $R$ contains critical points, the behavior may be
more complicated:  The solution could tend towards a loop
composed of critical points connected by trajectories, or 
the solution could simply tend towards a critical point.
The theorem is more a qualitative statement rather than
something to help us in computations.  In practice, it is hard to either
find analytic solutions or to show that they exist for all time.
...
y' = -x + {(x^2+y^2-1)}^2 y.
\end{equation*}
A vector field and solutions with initial conditions
...
A direction plot of an unstable limit cycle in the xy-plane
where the circular limit cycle is on the unit circle, and all the arrows go
roughly clockwise.  A trajectory from the inside approaches the limit cycle,
while a trajectory starting just outside of the limit cycle spirals
out and goes around one
and a half times before peeling off and leaving the picture at the top.}
...
y' = -x + {(x^2+y^2-1)} y.
\end{equation*}
The unit circle is again a closed trajectory,
...
The unit circle is an asymptotically unstable limit cycle where
all trajectories spiral away.
If we consider the linear system $x' = y$, $y' = -x$, then all circles centered
at the origin (not just the unit circle) are closed trajectories,
but no trajectory spirals onto any
other, so none of the circles are limit cycles.
%By Poincar\'e--Bendixson, 
All these solutions are periodic.

By Picard's theorem (\thmvref{sys:picardthm}), at any point
in the plane, we can always find a solution to
\eqref{nlin:gensys} a little bit
...
bounded (it will stay inside the closed trajectory).  The moment
...
We next look for conditions when limit cycles (or periodic orbits) do not exist.
We assume
...
connected region.  So is the inside of the unit disc.  However,
...
$x=\cos(t)(1+e^{-t})$, $y=\sin(t)(1+e^{-t})$.  What can you say
...
origin, as
$x=\cos(t)(1+e^{-t})$, $y=\sin(t)(1+e^{-t})$ gets closer and closer to the unit
...
\begin{exercise}
Consider the system $r' = \sin(r)$, $\theta' = 1$ given
...
is a positive multiple of $\pi$ are closed
trajectories.
}
...
initial conditions of the planets can lead to very different configurations
...
Take the weather, perhaps the most well-known chaotic system.
A small change in the initial conditions
(the temperature at every point of the atmosphere for example) produces
drastically different predictions in a relatively short time, and so we cannot
...
Lorenz was looking at the relatively simple system:
\begin{equation*}
...
A diagram of a pendulum attached to the end of another pendulum.  The masses
at the ends are shown to be of different sizes and the two lengths are
different.}
\end{mywrapfigsimp}
...
will appear chaotic.  This type of chaotic system is the basis for a
number of office novelty desk toys.  It is simple to build a
version.  Take a piece of string.  Tie two heavy nuts at different
...
exactly).  When $c$ is not zero, the equation does not have a
closed-form solution, so we must resort to numerical solutions,
as is usual for nonlinear systems.
Not all choices of constants and initial conditions
...
We can still draw
trajectories, where $y=x'$ as usual.   
...
trajectories are close at first, but soon diverge significantly.
...
On the left, there is a plot of two trajectories in the phase space (the
space with x on the horizontal axis and x prime on the vertical axis).
The two trajectories start on the right of the plot very close to
each other and follow a similar path for a while but diverge very quickly
and are unrecognizable after a while.  The shape they follow is roughly
a large loop on the left and then a smaller loop on the right sometimes
making a smaller loop in between.  On the right is a plot of x over
t from 0 to 15 for the two solutions going up and down as they follow the
shapes described.  They are almost indistinguishable on
the left and almost the same until roughly t equals 7,
but by the time t is 15 they are visibly different and quite
far apart.}
...
A wide graph for t between 0 and 100 where the vertical x axis goes between
roughly -3 to 3.  The plot shows a function which goes up and down many times
roughly at the same overall rhythm and the peaks and troughs are very roughly at
the same height, but each peak has a very different shape with distinct
wiggles on each.}
...
\pagebreak[2]
Let us consider the long-term behavior.
In \figurevref{nlin:duf-long},
we plot the solution $x$ with initial conditions $(2,3)$ for
a longer period of time.
It is hard to see any pattern
in the shape of the solution except that it seems to oscillate, but each
oscillation appears quite unique.  Oscillation
...
the system is that we figured out what the long-term behavior was (not
...
\myquote{oscillation}
and what these oscillations have in common.
...
Two plots of thousands of points of the strange attractor.  Each one is
slightly differently deformed as the one on the right is phase shifted.
The points seem to be settling on thin bands that are bent and wrapped
around each other and distorted as if looking through a funhouse mirror.}
...
in the sequence
eventually get closer and closer, that is, they are attracted.  The
...
either goes towards infinity, is periodic, approaches a loop,
or perhaps goes towards a critical point.
Hardly the chaotic
...
See \figurevref{nlin:lorenz} for a sample trajectory,
which is now a curve in three-dimensional space.
\begin{myfig}
\capstart
\diffyincludegraphics{width=3in}{width=4.5in}{nlin-lorenz}{%
A trajectory in a 3 dimensional xyz space.
It is roughly two loops meeting at an angle with the trajectory going
around each loop one or more times then switching
seemingly randomly to the other loop and then back again after one or 
more loops, each one of different radius.  Some people describe this picture
as resembling a butterfly.}
...
A graph of a function in the tx-plane for t from 0 to 15 and
x between roughly minus 18 to 18.
The function starts slightly above the t-axis and makes a sharp peak up
above the t-axis,
then a sharp peak down, then a sharp peak up, then 6 sharp peaks
down with the peaks starting small and growing, then two sharp peaks up
then one sharp peak down, then two sharp peaks up, then sharp peak down,
then one sharp peak up, then two sharp peaks down, then two sharp peaks up.
The peaks are different sizes and slightly different shapes, there does
not seem to be much of a pattern that one can see.}
...
Roll a die to pick one of the $p_1$, $p_2$, or $p_3$
randomly (for example 1 and 4 mean $p_1$, 2 and 5 mean $p_2$, and 3 and 6
mean $p_3$).  Suppose we picked $p_2$.  Let $p_{\text{new}}$ be the
...
A diagram of the so-called Sierpinski triangle composed of dots in
the xy-plane.  The vertices of the triangle are at (0,0), (1,0),
and (0.5,0.8).  There is a smaller similar upside down empty triangle with
the middle of the sides of the big triangle as vertices.
The remaining 3 upright triangles are not filled, but each has an
empty triangle like that in the middle and the three upright triangles
repeat the pattern forever.  The entire triangle then appears as if it is
made up of infinitely many upside down empty triangles.}
...
Use computer software (such as Matlab, Octave, or
perhaps even a spreadsheet) to plot the solution
of the given forced Duffing equation with Euler's method.  Plot the
...
\end{exercise}
\exsol{%
Critical points: $(0,0,0)$, $(6\sqrt{2},6 \sqrt{2}, 27)$,
$(-6 \sqrt{2},-6 \sqrt{2}, 27)$.
Linearization at $(0,0,0)$ using $u=x$, $v=y$, $w=z$ is
$u' = -10u+10v$, $v'=28u-v$, $w'=-(\nicefrac{8}{3})w$.
Linearization at $(6 \sqrt{2},6\sqrt{2},27)$ using $u=x-6\sqrt{2}$,
$v=y-6\sqrt{2}$, $w=z-27$ is
