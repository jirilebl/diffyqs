...
\sectionnotes{1.5--2 lectures\EPref{, \S7.1 in \cite{EP}}\BDref{,
...
$s$ is thought of as the frequency.\footnote{Really, it is the \myquote{frequency} in terms
...
finding out what will happen in the future
(the Laplace transform is one place
where it is safe to ignore the past).
Let us compute some simple
transforms.
...
\tablevref{lt:table} (see also \appendixref{laplacelist:appendix}).
...
properties of the integral.  For example, if $C$ is a constant, then
\begin{equation*}
...
\end{equation*}
So we can \myquote{pull out} a constant from the transform.
...
discontinuities like the Heaviside function.  The Laplace transform, however, does
...
\begin{exercise}
Find the Laplace transform of $te^{-t}$.  Hint: Shifting property.
\end{exercise}
\exsol{%
$\frac{1}{{(s+1)}^2}$
}
...
\end{equation*}
We will take the Laplace transform of both sides of the equation.
...
then applying the Laplace transform may not
give us an algebraic equation.
...
A plot of the unit step function on the interval from minus 1 to 1.
The function is zero for negative t and 1 for positive t.}
...
transform?  We saw that (for $a \geq 0$)
\begin{equation*}
...
The function $f(t)$ is not periodic and is defined piecewise; no problem for Laplace.
Imagine a
...
again
starting at $t=5$ (recall that $f(t)$ represents the derivative of
the voltage in the RLC circuit).
...
A plot of a function for t equals 0 to 20.  The function starts at zero
and stays at zero until t equals 1 when it curves upwards and follows a slightly
deformed sinusoid curve before settling into a regular sinusoid curve
past t equals 5.}
...
Then $f(t)$ is usually thought of as the input of the system and $x(t)$ is
...
We take the Laplace transform of the equation to obtain, for some $A(s)$,
\begin{equation*}
A(s) X(s) = F(s) .
\end{equation*}
Solving for the ratio $\nicefrac{X(s)}{F(s)}$ gives the so-called
...
H(s) = \frac{X(s)}{F(s)} .
\end{equation*}
In other words, $X(s) = H(s) F(s)$.  We get an algebraic dependence of
...
A feature of the Laplace transform is that it is able to deal
with integral equations.  These are equations in which integrals rather than
...
\begin{example}
Suppose $f(t)$ is a version of the \emph{\myindex{sawtooth}}, that is,
let
$f(t) = t$ for $0 \leq t < 1$
...
\begin{exercise}
Show the \myquote{differentiation of the transform} property.  Suppose
...
justify the second shifting property: 
$\mathcal{L} \bigl\{ f(t-a) \, u(t-a) \bigr\}
= e^{-as} \mathcal{L} \bigl\{ f(t) \bigr\}$.
\end{exercise}

\begin{exercise}
Consider the mass-spring system with a rocket from
...
\task Solve $x''+x=f(t)$, $x(0)=0$, $x'(0) = 0$ using the Laplace transform.
\end{tasks}
\end{exercise}
...
for the input function $f(t) = t$ gives the output
$x(t) = 2e^{-t} + te^{-t} + (t-2)$.
\begin{tasks}
\task Find $F(s)$, $X(s)$, and the transfer function $H(s)$.
\task If the input is instead $f(t) = \sin(t)$, find the new output $x(t)$.
\end{tasks}
\end{exercise}
...
\begin{exercise}
Suppose $Lx = f(t)$, $x(0)=0$, $x'(0)=0$
for the input function $f(t) = 4$ gives the output
...
a different type of \myquote{product.}
Take
two functions $f(t)$ and $g(t)$ defined for $t \geq 0$,
and define the \emph{\myindex{convolution}}%
\footnote{%
For those who have seen convolution before, you may have
seen it defined as
$(f * g)(t) =
...
Let $c$ be a constant and $f$, $g$, and $h$ be functions.
It is a calculus exercise to verify that
\begin{align*}
& f * g = g * f , \\
& (c f) * g = f * (c g) = c (f*g) , \\
& (f+g) * h = f * h + g * h , \\
& ( f * g ) * h = f * ( g * h ) .
\end{align*}
The first property means that $(f * g)(t) =
\int_0^t f(\tau) g(t-\tau) \, d\tau = 
...
.
\end{equation*}
We recognize the two entries of \tableref{ltd:table}:
\begin{equation*}
\mathcal{L}^{-1} 
\left\{
\frac{1}{s^2} \right\} 
...
We can now see how the Laplace transform
...
An integral equation\index{integral equation}
involving convolution
that comes up, for example, when studying systems with memory effects
...
\end{equation*}
Here $f(t)$ and $g(t)$ are known functions and $x(t)$ is an unknown we
wish to solve for.
We have $x(t) = f(t) + (g * x)(t)$, so the question is:
If convolution is
like a product, can we also \myquote{divide} to solve this equation?
...
The simplest kind of a pulse is a
...
A graph of a rectangular pulse shown from t equals 0 to t equals 3.
The function is zero until t equals 0.5, then it equals 2
between t equals 0.5 and t equals 1.0, and then it is zero again from
t equals 1.0 onwards.}
...
The Laplace transform of a rectangular pulse is
\begin{equation*}
\begin{split}
...
\end{equation*}
That is, we have a pulse of \myquote{unit mass.}
This way for any $b$ the pulse has the same \myquote{oomph.}
For such a pulse,
\begin{equation*}
{\mathcal{L}} \bigl\{ \varphi(t) \bigr\}
=
...
of $\frac{1-e^{-bs}}{bs}$
as $b \to 0$ is $1$, so we are looking for a \myquote{function} whose Laplace
transform is $1$.
...
each having an integral of $1$.  For example, 
...
\begin{example}
Let us compute ${\mathcal{L}}^{-1} \left\{ \frac{s+1}{s} \right\}$.
So far, we only computed the inverse transform of
proper rational functions in the $s$ variable.
...
$L x = f(t)$,
we think of $f(t)$ as the input, and $x(t)$ as the output.
...
However, we really want to think that $x(t)=0$ for $t \leq 0$, so $x(t)$
has a \myquote{corner} at $t=0$, that is,
$x'$ has a jump discontinuity there,
which is what produces the $\delta(t)$ when we take $x''$.
See \remarkref{remark:deltaderivofjump}.
The initial condition really is $x'(0-) = \lim_{t \uparrow 0} x'(t) = 0$.
\end{remark}
...
For functions $f(t)$ and $h(t)$ that are zero for negative $t$,
\begin{equation*}
(f * h)''(t) =
\frac{d^2}{dt^2}\left[
\int_0^t
...
= (f * h'')(t) .
\end{equation*}
We differentiate twice under the
integral\footnote{%
You may be worried about the fact that our $h$
is not really twice differentiable, in fact, $h'(0)$
does not exist as a normal derivative, and $h''$ involves
the $\delta$ function.
You can pretend that $h$ is twice differentiable
for this computation and either apply the standard
Leibniz rule (that $h(t)=0$ for $t < 0$ is important) or
note that for such functions,
the integral is actually over
$(-\infty,\infty)$ rather than over $[0,t]$.}.
...
A quite different
example application where the delta function appears is in
...
A diagram of a beam along the x-axis being held at fixed points at
x equals 0 and at some positive x.  The beam is getting bent down
by a weight labeled F times delta times quantity x minus a
in the center at one point.}
...
Suppose that the length of the beam is 2, and $EI=1$ for
simplicity.  Further suppose that a force $F=1$ is applied at $x=1$.
...
in the $x$ variable rather than the $t$ variable.  We again denote the
transform of $y(x)$ by $Y(s)$.
\begin{equation*}
...
second shifting property \eqref{ltd:sseq} for the first
term:
\begin{equation*}
...
\end{equation*}
Hence, $C_2 = \frac{1}{2}$.
Solving for $C_1$ using the first equation, we obtain
...
suppose that a force $F=1$ is applied at $x=\nicefrac{3}{4}$ in the downward
...
problem.
It is best to see the procedure in an example.
...
A diagram of the xt-plane with the first quadrant shaded
and labeled y sub t equals minus alpha times y sub x,
the x-axis labeled y equals 0,
and the t-axis labeled y equals C.}
\caption{Transport equation on a half line.\label{lt:half-infinite-goo-river}}
\end{mywrapfig}

A physical setup of this equation is a river of viscous goo,
...
\end{equation*}
The transform of a derivative with respect to $x$ is just the derivative of 
...
\end{equation*}
We are not done.
We have $Y(x)$, but we really want $y(x,t)$.
We transform the $s$ variable back to $t$.
Let 
\begin{equation*}
...
factory.
What the equation does is to try to move the initial condition to the right
at speed $\alpha$,
and so it moves the boundary condition (the wavefront) too.

\begin{myfig}
\capstart
\inputpdft{half-infinite-goo-river-wavefront}{%
A diagram of the first quadrant of the xt-plane (positive x and t).
The x-axis, which is the horizontal, is labeled y equals 0 and the t-axis
is labeled y equals C.  A diagonal straight line from the origin up
and to the right is labeled 'wavefront, slope 1 over alpha'.  The region
above this line is shaded dark and labeled y equals C.
The region below the line is shaded lightly and labeled y equals 0.}
\caption{Wavefront of toxic substance is a line of slope
$\nicefrac{1}{\alpha}$.\label{lt:half-infinite-goo-river-wavefront}}
\end{myfig}


Shhh\ldots $y$ is not differentiable.
...
The Laplace transform is very good with constant-coefficient equations.  One
...
A diagram of the xt-plane where the first quadrant is shaded.
The horizontal x-axis is labeled y equals 0 and the vertical t-axis is
labeled y equals sin of t.  The point (0,1) on the t-axis is labeled
as y of (0,1) equals sin of 1 and the point (1,0) on the x-axis is labeled
y of (1,0) equals 0.  The two points are connected with a line labeled as
t equals minus x plus 1
and also with 'y is constant along this characteristic curve'.}
...
since $x > 0$ in the region of interest,
\begin{equation*}
\lim_{s \to \infty}
\frac{1}{s^2+1} e^{sx}
= \infty \neq 0 .
...
& y(x,0) = 0 .
\end{align*}
This problem corresponds to a half-infinite insulated rod with a given
heat flux at one end.
The setup would come up in real life in the case of a
sufficiently long rod, where we would not consider a long enough time
for our solution to notice that we do not have an infinite rod but just a
very long one.
In any case, the real-life situation imposes other conditions
on our solution $y$.
For example, we will assume that the solution is bounded.
This boundedness condition will stand in for a boundary condition at the
infinite end.
Boundedness
is also sufficient for the Laplace transform in the $t$ variable to exist.
...
\end{equation*}
Note that $A$ and $B$ depend on $s$.
As $y$ is bounded, then $Y(x)$ is bounded for any fixed $s > 0$,
so for $Y(x)$ to stay bounded as $x \to \infty$, we must have $A=0$.

Now consider the boundary condition at $x=0$.
...
Hint: Note that for any fixed $s > 0$,
$e^{sx}$ blows up as $x \to +\infty$
and
$e^{-sx}$ blows up as $x \to -\infty$.
\end{exercise}
...
Assume that $y$ is bounded.
Give the answer as a convolution.
\end{exercise}
\exsol{%
$
y(x,t) =
