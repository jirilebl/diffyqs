...
Consider a PDE in two independent variables.  The idea is to use the
...
The PDE becomes an ODE that we know how to solve.
...
\caption{Transport equation on a half infinite line.\label{lt:half-infinite-goo-river}}
\end{mywrapfig}
...
We have a function of two variables, $y(x,t)$, that we wish to transform. 
We fix one variable and transform the other.
Let us transform the $t$ variable.
For convenience, we treat the transformed $s$
variable as a parameter, as there will be no derivatives in $s$.
That is, we write $Y(x)$ for the transformed function,
and treat it as a function of $x$, with $s$ being a parameter.
\begin{equation*}
Y(x)
...
and so we use the rules for transforming derivatives:
\begin{equation*}
...
solution.  Notice the line of slope $\nicefrac{1}{\alpha}$ indicates
...
(nobody ever seems to notice this).  How could we plug something that's not
...
Let us again transform the $t$ variable.  We again write $Y(x)$ for the
transformed function.  The equation becomes (using that $y(x,0) = 0$)
\begin{equation*}
...
In the above example, we can also detect when the problem is
\emph{\myindex{ill-posed}}
in the sense that it has no solution.
For example, if in the above example we change the equation to the simpler
\begin{align*}
...
So for example
the curve $t=-x+1$ intersects at $(1,0)$ and $(0,1)$.  Then
$y(1,0) = 0$, but $y(0,1) = \sin(1) \not= 0$.
...
\end{equation*}
Firstly $A=0$ since otherwise $Y$ does not decay to zero again as $s \to \infty$.
