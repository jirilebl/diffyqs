...
Maybe a simpler example (variable side codintions on a finite rod? perhaps)  Maybe the first order one first?  What about
...
DRAW the setup.  Think say a river of moving goo (no diffusion),
where $u$ is concentration of
some toxic substance (it's a goo already, we're not hurting the environment
much more).  At $x=0$, $t=0$, we start pouring the toxic substance to the
river so that at
$x=0$ the concentration is always $C$.
...
DRAW PLOT Slope of wavefront is $1/\nu$.
