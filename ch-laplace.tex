...
$\mathcal{L} \bigl\{ f(t) \bigr\} = F(s)$ for the Laplace transform of $f(t)$.  It is common to write lower case letters for
...
\toprule
%mbxFIXME
%the hspace might not work with the conversion script check
...
to find $f(t)$ such that $\mathcal{L} \bigl\{ f(t) \bigr\} = F(s)$, we need to first
...
\sectionnotes{? lecture\EPref{, \S?.? in \cite{EP}}\BDref{,
\S?.? in \cite{BD}}}
...
Suppose we have a PDE in two independent variables.  The idea is to use the
Laplace transform to transform one of the variables (taking the transform
of everything in sight), and derivatives in that variable become
multiplications by $s$.  Therefore, a PDE becomes an ODE which can be
solved.  Then we transform things back.
It is best to see it on an example.

\begin{example}
Consider the first order PDE
\begin{equation*}
u_t = - \alpha u_x, \qquad \text{for } x > 0, t > 0,
\end{equation*}
with side conditions
\begin{equation*}
u(0,t) = C, \qquad u(x,0) = 0 .
\end{equation*}
This equation is 
...
A physical setup of this equation is a river of goo (we are assuming no
diffusion, or at least very little diffusion).  The function
$u$ is the concentration of
some toxic substance (it's a goo already, we're not hurting the environment
much more).  The variable $x$ denotes position where $x=0$
is the location of a factory spewing the toxic substance into the
river.  The toxic substance is poured into the river so that at $x=0$ the
concentration is always $C$.  We wish to see what happens past the factory,
that is at $x > 0$.  We let $t$ be the time.  Finally we are assuming the
factory initiated operation at $t=0$, so that at $t=0$ the river is just
pure goo.

FIXME: picture of the setup

We have a function of two variables $u(x,t)$ that we wish to transform, so
we fix one variable and transform the other.  Let us transform the $t$
variable.  For convenience, we will treat the transformed $s$
variable as a parameter,
as there will be no derivatives in $s$.  That is, the transformed function
will be a function of $x$ with depending on a parameter $s$
That is, we write $U(x)$ for the transformed function:
\begin{equation*}
U(x)
= {\mathcal L} \bigl\{ u(x,t) \bigr\}
= \int_0^\infty u(x,t) e^{-st} ~ds .
\end{equation*}
The $x$ derivative is not difficult to compute:
\begin{equation*}
{\mathcal L} \bigl\{ u_x(x,t) \bigr\} =
\int_0^\infty u_x(x,t) e^{-st} ~ds
=
\frac{d}{dx} \left[\int_0^\infty u(x,t) e^{-st} ~ds \right]
=
U'(x) .
\end{equation*}
The derivative in $t$ is the derivative of the transformed variable
\begin{equation*}
{\mathcal L} \bigl\{ u_t(x,t) \bigr\} 
=
sU(x) - u(x,0) .
\end{equation*}

As $u(x,0)=0$, then in our specific case,
${\mathcal L} \bigl\{ u_t(x,t) \bigr\} = sU(x)$.  We transform the equation
to find
\begin{equation*}
sU(x) = -\alpha U'(x) .
\end{equation*}
We need an initial condition for this ODE.  This initial condition is the
other side condition of the PDE, the one that depends on $x$.  Since we have
transformed everything we need to transform this condition too
\begin{equation*}
U(0) = 
{\mathcal L} \bigl\{ u(0,t) \bigr\} 
=
...
We solve the ODE $sU(x) = -\alpha U'(x)$, $U(0) = \frac{C}{s}$ to find
\begin{equation*}
U(x) = \frac{C}{s} e^{-\frac{s}{\alpha} x} .
\end{equation*}
We are not done, we have $U(x)$, but we really want $u(x,t)$.  We need
to transform the $s$ variable back to $t$.
Let us use
\begin{equation*}
H(t) = \begin{cases} 0 & \text{if $t < 0$}, \\ 1 & \text{otherwise}
\end{cases}
\end{equation*}
for the Heaviside function,
since we clearly cannot use $u$.
As
\begin{equation*}
{\mathcal L} \bigl\{ H(t-a) \bigr\} =
...
FIXME: Plot,  slope of wavefront is $1/\alpha$.

What the equation does is simply move the initial condition to the right at
speed $\alpha$.
Shhh ... $u$ is not differentiable, it is not even continuous
(nobody ever seems to notice this).
Well, just think of a function very very close to $u$ that is differentiable.
Or, if you recognize the derivative of the Heaviside function as the delta
function, then all is well too:
\begin{equation*}
u_t (x,t) = \frac{\partial}{\partial t} \left[
C  H\bigl(t-\nicefrac{x}{\alpha}\bigr)
\right]
=
C  H'\bigl(t-\nicefrac{x}{\alpha}\bigr)
...
and
\begin{equation*}
u_x (x,t) = \frac{\partial}{\partial x} \left[
C  H\bigl(t-\nicefrac{x}{\alpha}\bigr)
\right]
=
- \frac{C}{\alpha}  H'\bigl(t-\nicefrac{x}{\alpha}\bigr)
=
...
FIXME: Just a dump from my draft notes:

Maybe a simpler example (variable side conditions on a finite rod? perhaps)  Maybe the first order one first?  What about
infinite $x$ on both sides.
...
$$
U(x) = A e^{\sqrt{s}x} + B e^{-\sqrt{s} x}
$$

Now how about the boundary conditions.  Firstly $A=0$ since otherwise
unbounded.
 We transform $U'(0) = F(s)$ and
so
$-\sqrt{s}B = F(s)$, and
$$
U(x) = - F(s) \frac{1}{\sqrt{s}} e^{-\sqrt{s} x}
$$

If we look up the inverse transform in a table (or we spend the afternoon
doing calculus) we find 
$$
{\mathcal L}^{-1}[e^{-\sqrt{s} x}] =
\frac{x}{\sqrt{4\pi t^3}} e^{\frac{-x^2}{4t}}
$$
$$
{\mathcal L}^{-1}[\frac{1}{\sqrt{s}} e^{-\sqrt{s} x}] =
...
Laplace is often useful in places where separation of variables fails.
