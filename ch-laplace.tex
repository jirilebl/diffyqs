...
diffusion).  The function
$y$ is the concentration of
some toxic substance\footnote{It's a river of goo already, we're not hurting the environment
...
FIXME: Maybe a simpler example (variable side conditions on a finite rod? perhaps)
Maybe the first order one first?  What about
infinite $x$ on both sides.

\begin{example}
Let us use Laplace for the following problem:
\begin{align*}
& y_t = y_{xx}, \qquad 0 < x < \infty, \\
& y_x(0,t) = f(t), \\
...
Now consider the boundary conditions.  Firstly $A=0$ since otherwise
$Y$ is unbounded in $x$, so FIXME
