...
FIXME: the rest is a dump of notes

Let's use Laplace to solve this since we have just learned it:

$U(x) = {\mathcal L} \bigl\{ u(x,t) \bigr\}$
in the $t$ direction and we'll treat $s$ as parameter again.  $u(x,0) = 0$ and
so

$sU(x) = -\alpha U'(x)$, $U(0) = \frac{C}{s}$.
So
$$
U(x) = \frac{C}{s} e^{-\frac{s}{\alpha} x}
$$
For the inverse transform we use the Heaviside function
$$
...
DRAW PLOT Slope of wavefront is $1/\alpha$.

Shhh ... $u$ is not differentiable (nobody ever seems to notice this).
Well, just think of something very very close to $u$ that is differentiable.
