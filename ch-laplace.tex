...
A plot of the unit step function on the interval from -1 to 1.
...
A plot of a function for t=0 to 20.  The function starts at zero
and stays at zero until t=1 when it curves upwards and follows a sightly
...
A graph of a rectangular pulse shown from t=0 to t=3.
The function is zero until t=0.5, then it equals 2
between t=0.5 and t=1.0, and then it is zero again from t=1.0 onwards.}
...
x=0 and at some positive x.  The beam is getting bent down
by a weight labeled F delta(x-a) in the center at one point.}
...
and labeled y_t = - alpha y_x, the x-axis labeled y=0,
and the y-axis labeled y=C.}
...
The x-axis, which is the horizontal, is labeled y=0 and the t-axis
is labeled y=C.  A diagonal straight line from the origin up
and to the right is labeled 'wavefront, slope 1/alpha'.  The region
above this line is shaded dark and labeled y=C.
The region below the line is shaded lightly and labeled y=0.}
...
The horizontal x-axis is labeled y=0 and the vertical t-axis is
labeled y=sin(t).  The point (0,1) on the t-axis is labeled
as y(0,1)=sin(1) and the point (1,0) on the x-axis is labeled
y(1,0)=0.  The two points are connected with a line labeled as t=-x+1
