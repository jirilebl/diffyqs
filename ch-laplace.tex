...
= {\mathcal{L}}^{-1}\bigl\{ H(s)F(s) \bigr\}
= (h * f)(t)
= \int_0^t h(\tau) f(t-\tau) ~ d\tau .
\end{equation*}
Hence,
...
we found that
when the input is $f(t)$, the solution to $Ly = f(t)$
is given by
\begin{equation*}
y(t) = 
\int_0^t
...
$x(t)$ are zero for negative $t$.}.
Suppose that $x(t)$ is the impulse response.
