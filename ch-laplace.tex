...
\begin{equation*}
y_t = - \alpha y_x, \qquad \text{for } x > 0, t > 0,
\end{equation*}
...
We have a function of two variables $y(x,t)$ that we wish to transform, so
...
As $y(x,0)=0$, then in our specific case,
...
We solve the ODE $sY(x) = -\alpha Y'(x)$, $Y(0) = \frac{C}{s}$ to find
\begin{equation*}
Y(x) = \frac{C}{s} e^{-\frac{s}{\alpha} x} .
\end{equation*}
We are not done, we have $Y(x)$, but we really want $y(x,t)$.  We need
...
\end{cases}
\end{equation*}
be the Heaviside function as before.
As
\begin{equation*}
{\mathcal L} \bigl\{ u(t-a) \bigr\} =
...
speed $\alpha$.
Shhh ... $y$ is not differentiable, it is not even continuous
(nobody ever seems to notice this).
Well, just think of a function very very close to $y$ that is differentiable.
