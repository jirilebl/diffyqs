...
where neither $\alpha_1=\alpha_2=0$
nor
$\beta_1=\beta_2=0$.
...
On the xy-plane a graph of two functions over the interval 0 to 7.
The first function, 1 over x,
comes down from the top of the picture on the left,
drops very quickly towards the x-axis, then keeps slowly approaching it but
never crossing it as x grows.  The second function is the tangent, which
starts at zero when x equals 0 and then quickly curves up and leaves the picture
before pi over 2 while crossing the graph of 1 over x the first time.  After pi
over 2,
the tangent reappears from the bottom of the picture curves right and then
left before leaving the picture again before 3 pi over 2.  It crosses the graph of
1 over x the second time in that interval.  After 3 pi over 2, it reappears from the
bottom and again goes up and crosses the graph of 1 over x the third time.}
...
assume it is positive) on $[a,b]$.  Two functions $f(x)$ and $g(x)$
are said to be
...
Its proof is very similar to that of the analogous 
...
boundary value problems.  The theorem does not help us solve the problem.
...
Diagram of a rigid beam laid along the x-axis in the xy-plane with one
end at the origin and the second end somewhere on the positive
x-axis.  The beam is bent upwards into the y direction indicating that
the graph of y gives the shape of the bent beam.}
...
Let us not worry about the physics\footnote{If
...
$T'' + \lambda a^4 T = 0$, physical intuition leads us
to the fact that if $\lambda$ is an eigenvalue then $\lambda > 0$:
We expect vibration and not exponential growth or
...
The timbre of a simply supported beam
...
$1,2,3,4,5,\ldots$.  For a simply supported beam, we get
only the square multiples $1,4,9,16,25,\ldots$, and so
we get something much closer to a pure sound.
The sound of a xylophone or vibraphone is, therefore, very different from
...
\end{aligned}
\end{equation*}
That is, you also have an initial velocity.  Find a series solution.  Hint:
...
0$.   Suppose you know that the initial shape of the beam is the graph of
$\sin^2(\pi x)$, and the initial velocity is equal to $x(10-x)$.
...
string.  Or perhaps from a jet engine.  For simplicity, assume a nice pure
...
$F_0 \cos (\omega t)$ as force per unit mass.
We will call this the forcing function.  Then our wave
...
\end{equation*}
We then find a solution $y_c$ of \eqref{sps:freevib}.  If we add the two
...
We get to the point that we skipped.  Suppose 
$\sin ( \frac{\omega L}{a} ) = 0$.  This means that
...
The calculation of resonance frequencies above explains why a string begins
to vibrate if an
...
frequency.  However, a glass has a much purer sound, i.e.\ it is more like a
...
its Fourier series and apply the result above.  You may also need to solve
...
Suppose $F_0 = 1$, $\omega = 1$, $L=1$, and $a=1$.
Let us find a solution $y$ to \eqref{sps:forcedeq}
satisfying the zero boundary conditions
\eqref{sps:forcedinitcond}.  First we find
\begin{equation*}
y_p(x,t) =
\left(
\cos (x) -
...
+1 ,
\end{equation*}
and after differentiating $y_p$ in $t$, we see that 
...
the formula that we use to define $w(x,0)$ is not odd,
hence it is not a simple matter of plugging in the expression for $w(x,0)$
to the d'Alembert
formula directly!  You must define $F$ to be the odd, 2-periodic
extension of $w(x,0)$.  Then our solution $y = y_p+y_c$ to
\eqref{sps:forcedeq} subject to \eqref{sps:forcedinitcond}
is
...
A plot of a surface above the xt-plane where x ranges from 0 to 1 and t
ranges from 0 to 5.  The function y is zero on 3 sides, that is at t equals 0
and when x equals 0 or x equals 1.  Inside its domain as t grows there is a bump going up
to a bit above 0.2, then there are two bumps going below the xt-plane,
the second one seeming a little more pronounced.
Then the function starts going into another bump upwards.}
...
A diagram of the ground with a person standing on it, and an arrow
downward marked with 'depth x' indicating that x means the depth below
ground.}
...
temperature, and
$t=0$ is midsummer (put
a negative sign above to make it midwinter if you wish).
The constant $A_0$ gives 
the typical variation for the year:
