...
Undetermined coefficients require $f(x)$ and its derivatives
...
The technique works for general linear equations of any order,
but we stick to constant coefficient second order equations.

\medskip
\pause

Suppose $Ly= 0$ (the associated homogeneous equation) has the general
solution
\[
C_1 y_1 + C_2 y_2 ,
\]
...
In general, the procedure for finding $y_p$ using variation of parameters (2nd
order) is
...
\]
\pause
3) Integrate to get $u_1$ and $u_2$, then $y_p = u_1 y_1 + u_2 y_2$.
