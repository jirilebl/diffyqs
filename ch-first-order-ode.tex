...
In the above examples, $b$ was always $1$ meaning we got $s=t$ for the 
characteristic example, if $b$ is not constant, your expression for $s$
will become more complicated.

One thing to keep in mind is that we would get into trouble if the
coefficient in front of $u_t$, that is the $b$, is every zero.
...
if necessary, so that positive $s$ means positive $t$.  As $b$ should never
be zero, you could also always divide by $b$ to make the coefficient
always $1$, however, that may make the other coefficients more complicated.
