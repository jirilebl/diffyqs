...
Notice that the \emph{initial condition}\index{initial condition for a PDE}
$u(x,0) = f(x)$ is now a function rather than just a number.
The $a$, $b$, and $c$ do not have to be constant; the method
we apply works with nonconstant coefficients, although the computations
can get difficult. So we concentrate
...
The idea is that we will find lines along which we just solve an ODE.
...
\end{align*}
So the equation in the coordinates $\xi$ and $s$ becomes
\begin{equation*}
...
u_s = 0 .
\end{equation*}
That's trivial to solve.  We integrate to get a function that does
...
u(x,t) = f(x+\alpha t)
\end{equation*}
Simply everything is moving left at speed $\alpha$.
The curve given by the equation
\begin{equation}
\xi = \text{constant}
\end{equation}
is called the characteristic.  In this case, the solution does not change
along the characteristic.

FIXME: DRAW CURVES.

Note that in the $(x,t)$ coordinates, writing $t =
...
characteristic coordinates, let us call the new coordinates $(\xi,s)$.
