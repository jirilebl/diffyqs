...
A plot of y=1/(1-x) which is a line starting at a positive value on
the left, then rising towards a vertical asymptote at x=1.  Then from
...
A grid of line segments whose slope at each point is given by xy.}
...
A grid of line segments with the slope given by 1/x.  Overlaid
...
and 60.  A vertical line at t=1 intersects the solution
at temperature 85.  A vertical line at t=9.21 intersects the
solution at temperature 60.  The right hand plot is over longer
period of time to t=80.  The solution exponentially decays
...
at the bottom.  The top input is labeled as 5 L/min, 0.1 kg/L.
The bottom output is labeled as 3 L/min.  Inside the liquid is
labeled as 60 L and 10 kg salt.}
\end{mywrapfigsimp}
...
A graph of a solution from t=0 to t=20
...
%mbxlatex A graph of a solution from t=0 to t=20
...
A plot for t from 0 to 20 and x from -10 to 10 with 
a grid of lines with slopes of 0.3(5-x) given.  These 
slopes are going up below the x=5 line and are going down
above the x=5 line.  The slopes are steeper the further away
from x=5 line we are.  Several solutions are shown which
are lines starting at various points on the left and then
all converging towards the x=5 line as t increases.}
...
A plot for t from 0 to 20 and x from -5 to 10 with
a grid of lines with slopes of 0.1x(5-x) given.  These 
slopes are going down below the x=0 line and above the x=5
line.  Between x=0 and x=5 the slopes are going up.
Several solutions are shown which when starting below x=0 just keep
going down and leave the plot.  When the solution is above x=0,
then it rises and then approaches the x=5 line.  When the solution
starts above x=5, it goes down towards and approaches the x=5 line.
The lines x=0 and x=5 are also solutions.}
 \caption{The slope field and some solutions of
...
marked as x=0 for the lower one and x=5 on the upper one.
There are triangular arrows on the line.  Below x=0 the arrow points down,
between the x=0 and x=5 ticks the arrow points up, and above the x=5 tick
...
a grid of lines with slopes of 0.1x(8-x)-1 given.  These 
slopes are going down below roughly x=1.55 and above roughly
x=6.45.  Between these two lines the slopes are going up.
Several solutions are shown, which when starting below x=1.55 keep
going down and leave the plot.  When the solution starts above
x=1.55, then it rises and then approaches the line at roughly x=6.45.
When the solution starts above x=6.45, it goes down towards and
approaches this line at roughly x=6.45.  The two horizontal lines
are also solutions.}
 \caption{The slope field and some solutions of
...
A plot for t from 0 to 20 and x from 0 to 10 with
a grid of lines with slopes of 0.1x(8-x)-1.6 given.  In this
plot there is only one horizontal line at x=4.  The slopes are all
...
equilibrium solution as a horizontal line x=4. Another solution
going down from above and approaching the x=4 is drawn and a solution
starting slightly below the x=4 line and then going down and
...
A plot for t from 0 to 20 and x from 0 to 10 with
a grid of lines with slopes of 0.1x(8-x)-2 given.  In this
plot, there are no horizontal lines and all the slopes are down.
...
Two plots plot in the xy-plane of a slope field with slopes y^2/3
in the region where x is between -1 and 3 and y is between 0 and 3.
...
a green line starting at (0,1) of slope 1/3, which is the slope at
(0,1) and going right until x=1.  In the right plot, a second
line is added starting at the end of the first line with slope equal 
to the slope of the slope field and going further right until x=2.}
...
A plot in the xy-plane for x between roughly -13 to 13 and y
between -10 to 10.  Several circles centered at the origin, (0,0),
are drawn.}
...
A plot in the xy-plane for x between -5 to 5 and y from -10 to 10
...
The one starting at the origin is marked as xi=0, the one to the right
is marked as xi=1, and then the third one to the right of that is marked
xi=2.}
...
Two plots in the plane for x between -3 to 3 and the dependent
...
except for a sharp peak up to level 1 when x=0.
...
sits on the x-axis is marked as x=xi and also s=0.  The general point
on the curve is marked as (x(s),t(s)).  The entire curve is marked as
xi=constant.}
...
A surface plot of a function u(x,t) for x between -3 and 3 and t
between 0 and 3.  When t=0, the surface looks line a
...
At t=3 it looks almost completely
straight from a bit less than -3 at x=-3 and goes up to a bit below 2.}
