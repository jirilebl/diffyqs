...
\begin{myfig}
\capstart
%mbxalt A plot of y=1/(1-x) which is a line starting at a positive value on
%mbxalt the left, then rising towards a vertical asymptote at x=1.  Then from
%mbxalt below there is another line that rises from the vertical asymptote
%mbxalt and approaches the x-axis.
...
\begin{myfig}
\capstart
%mbxalt The xy plane with a short line segment of slope 3 at the point
%mbxalt (2,1.5).
...
\parbox[t]{3.0in}{
 \capstart
%mbxalt A grid of line segments whose slope at each point is given by xy.
...
\parbox[t]{3.0in}{
 \capstart
%mbxalt A grid of line segments whose slope at each point is given by xy.
%mbxalt Overlaid are three red lines going through these line segments,
%mbxalt one is concave up above the x-axis, one is simply the x-axis,
%mbxalt and one is concave down below the x-axis.
...
\begin{myfig}
\capstart
%mbxalt A grid of line segments with the slope given by y.  Overlaid
%mbxalt are three red lines starting at various points on the left hand side that
%mbxalt all approach the x-axis as we move right.
...
\parbox[t]{3in}{
 \capstart
%mbxalt A grid of line segments with the slope given by 1/x.  Overlaid
%mbxalt are red lines starting at various points on the left hand side 
%mbxalt and the right hand side that go out the bottom of the picture
%mbxalt as they approach the y-axis.
...
\parbox[t]{3in}{
 \capstart
%mbxalt A grid of line segments with the slope given by 2 times the
%mbxalt square root of the absolute value of y.  These are lines that
%mbxalt go generally up except on the x-axis where they are horizontal.
%mbxalt There are two lines giving solutions, one horizontal and one
%mbxalt going up from the bottom of the plot and hitting the origin and
%mbxalt then leaving upwards.
...
\begin{myfig}
\capstart
%mbxalt Plot of the region of the xy-plane near the origin, and
%mbxalt there are two solutions plotted one concave up above the
%mbxalt x-axis and one concave down below the x-axis.
...
%mbxalt Two plots of the coffee temperature.  On the left, the downwards
%mbxalt solution is shown for time 0 to 12.5 as the temperature goes
%mbxalt from 89 to below 60.  There are horizontal lines at 89, 85,
%mbxalt and 60.  A vertical line at t=1 intersects the solution
%mbxalt at temperature 85.  A vertical line at t=9.21 intersects the
%mbxalt solution at temperature 60.  The right hand plot is over longer
%mbxalt period of time to t=80.  The solution exponentially decays
%mbxalt towards a vertical line at temperature 22.
...
\noindent
%mbxalt A diagram of a tank of liquid with a pipe showing water
%mbxalt flowing into the tank at the top and a pipe showing water leaving
%mbxalt at the bottom.  The top input is labeled as 5 L/min, 0.1 kg/L.
%mbxalt The bottom output is labeled as 3 L/min.  Inside the liquid is
%mbxalt labeled as 60 L and 10 kg salt.
\inputpdft{lin-tank}
\end{mywrapfigsimp}
...
%mbxlatex \capstart
%mbxlatex %mbxalt A graph of a solution from t=0 to t=20
%mbxlatex %mbxalt starting at 20 and then rising, at first slowly
%mbxlatex %mbxalt then speeding up and ending at roughly 11.86 at the
%mbxlatex %mbxalt right-hand side. 
...
\parbox[t]{3.0in}{
 \capstart
%mbxalt A plot for t from 0 to 20 and x from -10 to 10 with 
%mbxalt a grid of lines with slopes of 0.3(5-x) given.  These 
%mbxalt slopes are going up below the x=5 line and are going down
%mbxalt above the x=5 line.  The slopes are steeper the further away
%mbxalt from x=5 line we are.  Several solutions are shown which
%mbxalt are lines starting at various points on the left and then
%mbxalt all converging towards the x=5 line as t increases.
...
\parbox[t]{3.0in}{
 \capstart
%mbxalt A plot for t from 0 to 20 and x from -5 to 10 with
%mbxalt a grid of lines with slopes of 0.1x(5-x) given.  These 
%mbxalt slopes are going down below the x=0 line and above the x=5
%mbxalt line.  Between x=0 and x=5 the slopes are going up.
%mbxalt Several solutions are shown which when starting below x=0 just keep
%mbxalt going down and leave the plot.  When the solution is above x=0,
%mbxalt then it rises and then approaches the x=5 line.  When the solution
%mbxalt starts above x=5, it goes down towards and approaches the x=5 line.
%mbxalt The lines x=0 and x=5 are also solutions.
...
\begin{center}
%mbxalt A vertical line with two points marked with horizontal ticks
%mbxalt marked as x=0 for the lower one and x=5 on the upper one.
%mbxalt There are triangular arrows on the line.  Below x=0 the arrow points down,
%mbxalt between the x=0 and x=5 ticks the arrow points up, and above the x=5 tick
%mbxalt the arrow points down.
...
\begin{center}
%mbxalt On the left, there are thee possible phase diagrams with one
%mbxalt critical point marked as a horizontal tick.
%mbxalt The first one has the arrow above
%mbxalt and below the tick pointing away from the tick.  The second one has
%mbxalt both arrows pointing up, the third has both arrows pointing down.
%mbxalt These three diagrams are labeled as 'unstable'.  Then there is one
%mbxalt phase diagram with one critical point on the right where both
%mbxalt arrows are pointing towards the critical point.
%mbxalt This diagram is labeled as 'stable'.
...
\parbox[t]{3.0in}{
 \capstart
%mbxalt A plot for t from 0 to 20 and x from 0 to 10 with
%mbxalt a grid of lines with slopes of 0.1x(8-x)-1 given.  These 
%mbxalt slopes are going down below roughly x=1.55 and above roughly
%mbxalt x=6.45.  Between these two lines the slopes are going up.
%mbxalt Several solutions are shown, which when starting below x=1.55 keep
%mbxalt going down and leave the plot.  When the solution starts above
%mbxalt x=1.55, then it rises and then approaches the line at roughly x=6.45.
%mbxalt When the solution starts above x=6.45, it goes down towards and
%mbxalt approaches this line at roughly x=6.45.  The two horizontal lines
%mbxalt are also solutions.
...
\parbox[t]{3.0in}{
 \capstart
%mbxalt A plot for t from 0 to 20 and x from 0 to 10 with
%mbxalt a grid of lines with slopes of 0.1x(8-x)-1.6 given.  In this
%mbxalt plot there is only one horizontal line at x=4.  The slopes are all
%mbxalt going down above and below this line, more steeply the further away
%mbxalt from the line we are.  Three solutions are drawn, one is the
%mbxalt equilibrium solution as a horizontal line x=4. Another solution
%mbxalt going down from above and approaching the x=4 is drawn and a solution
%mbxalt starting slightly below the x=4 line and then going down and
%mbxalt dropping off the picture before we get to the right hand side.
...
\capstart
%mbxalt A plot for t from 0 to 20 and x from 0 to 10 with
%mbxalt a grid of lines with slopes of 0.1x(8-x)-2 given.  In this
%mbxalt plot, there are no horizontal lines and all the slopes are down.
%mbxalt The slopes are steeper at the top of the picture, they get slightly less
%mbxalt steep as we get to the middle of picture and then get steeper again at the
%mbxalt bottom.  Three solutions are drawn, all going down.
%mbxalt Solutions start steeply down at the
%mbxalt top of the picture then go down a bit more slowly in the middle part and
%mbxalt then become steep again at the bottom.
...
%mbxalt Two plots plot in the xy-plane of a slope field with slopes y^2/3
%mbxalt in the region where x is between -1 and 3 and y is between 0 and 3.
%mbxalt All the slopes are upwards with the ones further up being steeper.
%mbxalt In the left plot, the first step of Euler's method is shown as
%mbxalt a green line starting at (0,1) of slope 1/3, which is the slope at
%mbxalt (0,1) and going right until x=1.  In the right plot, a second
%mbxalt line is added starting at the end of the first line with slope equal 
%mbxalt to the slope of the slope field and going further right until x=2.
...
\begin{myfig}
\capstart
%mbxalt A plot in the xy-plane of a slope field with slopes y^2/3
%mbxalt in the region where x is between -1 and 3 and y is between 0 and 3.
%mbxalt The first two steps are shown the two green line segments as before,
%mbxalt but also the solution through (0,1) is drawn in red, which is curved
%mbxalt upwards and follows the slope field.  We can see that it stays above the
%mbxalt green line segments.
...
\capstart
%mbxalt A plot in the xy-plane for x between roughly -13 to 13 and y
%mbxalt between -10 to 10.  Several circles cetnered at the origin, (0,0),
%mbxalt are drawn.
...
\begin{myfig}
\capstart
%mbxalt A plot in the xy-plane for x between -5 to 5 and y from -10 to 10
%mbxalt is shown.  A blue line is drawn starting at (0,0) to the top right
%mbxalt corner at (5,10) is drawn with a red dot marking the initial point
%mbxalt drawn at (1,2).
...
\capstart
%mbxalt A diagram of the xt-plane with three lines drawn.  All three
%mbxalt line start on the x-axis and are going steeply up and to the right.
%mbxalt the one starting at the origin is marked as xi=0, the one to the right
%mbxalt is marked as xi=1, and then the third one to the right of that is marked
%mbxalt xi=2.
...
%mbxalt Two plots in the plane for x between -3 to 3 and the dependent
%mbxalt axis between a bit less than 0.0 and a bit above 1.0.  On the left plot,
%mbxalt there is a graph of a function which is mostly horizontal at level 0
%mbxalt except for a sharp peak up to level 1 when x=0.
%mbxalt On the right plot, it is the same exact peak but it is now centered
%mbxalt at x=1.
...
\begin{myfig}
\capstart
%mbxalt A diagram of the xt-plane with a sample characteristic curve
%mbxalt drawn starting on the x-axis and going up.  The point on the curve that
%mbxalt sits on the x-axis is marked as x=xi and also s=0.  The general point
%mbxalt on the curve is marked as (x(s),t(s)).  The entire curve is marked as
%mbxalt xi=constant.
...
\begin{myfig}
\capstart
%mbxalt A surface plot of a function u(x,t) for x between -3 and 3 and t
%mbxalt between 0 and 3.  When t=0, the surface looks line a
%mbxalt flat line then a peak and going back down towards a flat line.
%mbxalt As t grows the surface becomes, as t grows,
%mbxalt closer and closer to a sideways
%mbxalt flat surface that goes up as x grows.
%mbxalt At t=3 it looks almost completely
%mbxalt straight from a bit less than -3 at x=-3 and goes up to a bit below 2.
