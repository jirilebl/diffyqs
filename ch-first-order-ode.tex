...
%FIXME: we cover this again later, so just skip for now to improve flow.
%The theorem does not necessarily give us a global $F$
%defined everywhere in the plane.
%In general, we can only find the potential locally, near some initial point.
...
If we think back to the Poincar\'e Lemma, it does not guarantee such a function
anyway.  It only guarantees a potential function $F$ locally, that is, only in
...
has no holes, such as for instance, the entire $xy$-plane,
