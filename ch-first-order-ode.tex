...
Take $(y-x)y' = 0$, $y(0) = 0$.
\begin{enumerate}[a)]
\item Find two distinct solutions.
\item Explain why this does not violate Picard's theorem.  
\end{enumerate}
\end{exercise}
...
\end{align*}
Now solve for the initial condition, $0 = \tan(-2+C)$ to get $C=2$ (or $C = 2 +
\pi$, or $C = 2 + 2\pi$, etc.).  The solution we seek is, therefore,
\begin{equation*}
...
on the island.
\begin{enumerate}[a)]
\item Find the solution to the equation with the initial
condition.
\item
...
\begin{mywrapfigsimp}{1.35in}{1.65in}
\noindent
\inputpdft{lin-tank}
\end{mywrapfigsimp}
...
\begin{enumerate}[a)]
\item Find the concentration of toxic substance
...
\item In the long term, will the
...
\item Sketch typical solutions of the equation.
\item Find $\displaystyle \lim_{t\to \infty} x(t)$ for the solution with the initial condition
$x(0) = -1$.
\end{enumerate}
\end{exercise}
...
Take $x' = \sin x$.
\begin{enumerate}[a)]
\item Draw the phase diagram for $-4\pi \leq x \leq 4\pi$.  On this interval
...
\begin{enumerate}[a)]
\item Draw the phase diagram for $x' = f(x)$,
find the critical points, and mark them stable or unstable.
\item Sketch typical solutions of the equation.
\item Find $\displaystyle \lim_{t\to \infty} x(t)$ for the solution with the initial condition
$x(0) = 0.5$.
...
\begin{enumerate}[a)]
\item Construct the differential equation. 
\item Show that if $kM > h$, then
the equation is still logistic.
\item What happens when $kM < h$?
\end{enumerate}
\end{exercise}
...
\begin{enumerate}[a)]
\item Write down the differential equation.
\item Supposing $x(0) > 0$, that is, some people are infected at time $t=0$,
what is
...
Let $x'=(x-1)(x-2)x^2$.
\begin{enumerate}[a)]
\item Sketch the phase diagram and find critical
points.
\item Classify the critical points.
\item If $x(0)=0.5$ then find $\displaystyle \lim_{t\to\infty} x(t)$.
\end{enumerate}
\end{exercise}
\exsol{%
\\
...
Let $x'=e^{-x}$.
\begin{enumerate}[a)]
\item Find and classify all critical points.
\item Find $\displaystyle \lim_{t\to\infty} x(t)$ given any 
initial condition.
\end{enumerate}
\end{exercise}
\exsol{%
...
\begin{enumerate}[a)]
\item Find the differential equation for $x$.
\item What is the new limiting population?
\end{enumerate}
\end{exercise}
\exsol{%
...
\beta$.
\begin{enumerate}[a)]
\item Find the critical points, and classify them.
\end{enumerate}
%mbxFIXME
...
\item Describe what happens to the
...
\begin{enumerate}[a)]
\item Use Runge-Kutta (see above) with step sizes $h=1$ and $h=\nicefrac{1}{2}$
to approximate $y(1)$.
\item Use Euler's method with $h=1$ and
$h=\nicefrac{1}{2}$.
\item Solve exactly, find the exact value of
$y(1)$, and compare.
\end{enumerate}
\end{exercise}
...
Let $x' = 2t$, and $x(0)=0$.
\begin{enumerate}[a)]
\item Approximate $x(4)$ using Euler's method with step sizes 4, 2, and 1.
\item Solve exactly, and compute the errors.
\item Compute the factor by which the errors changed.
\end{enumerate}
\end{exercise}
\exsol{%
\\
...
\begin{enumerate}[a)]
\item Approximate $x(4)$ using Euler's method with step sizes 4, 2, and 1.
\item Guess an exact solution based on part a) and compute the errors.
\end{enumerate}
\end{exercise}
\exsol{%
a) 0, 0, 0
...
\begin{enumerate}[a)]
\item Use the improved Euler's method (see above) with step sizes $h=\nicefrac{1}{4}$ and $h=\nicefrac{1}{8}$
to approximate $y(1)$.
\item Use Euler's method with
$h=\nicefrac{1}{4}$ and $h=\nicefrac{1}{8}$.
\item Solve exactly, find the exact value of
$y(1)$.
\item Compute the errors, and the factors by which the errors changed.
\end{enumerate}
\end{exercise}
\exsol{%
...
\sectionnotes{1 lecture, normally not covered}
...
u_s = 0 .
\end{equation*}
That's trivial to solve.  If we treat $\xi$ as simply a parameter, we have
...
u(x,t) = f(x-\alpha t) .
\end{equation*}
Simply everything is moving right at speed $\alpha$ as $t$ increases.
...
In the $(x,t)$ coordinates, the characteristic curves have
the equation $t = \frac{1}{\alpha} ( x- \xi)$, and in fact lines.
...
the function $u(x,1)$, that is at time $t=1$.  Notice it is one unit to
the right.\label{fopde:transportfig}}
\end{myfig}
\end{example}

We can use similar idea in the more general case:
\begin{equation*}
...
So, along the characteristic curves (where $\xi$ is constant), we get a
...
\frac{du}{ds} + c u = g.
\end{equation}
Here we think of everything as a function of $\xi$ and $s$,
although
...
\begin{equation*}
\frac{du}{ds} + c u = 
\left( u_x \frac{dx}{ds} + u_t \frac{dt}{ds}\right) + c u =
a u_x + b u_t + c u = g .
\end{equation*}
...
we are solving along the characteristic is
\begin{equation*}
\frac{du}{ds} + u = s+ \xi .
\end{equation*}
The general solution of this ODE, treating $\xi$ as just a parameter, is 
$u = C e^{-s}+s+\xi-1$, for some constant $C$.
At $s=0$, our initial condition is that $u$ is
$e^{-\xi^2}$ as at $s=0$ we have that $x=\xi$.
Given this initial condition, we find $C=e^{-\xi^2} - \xi +1$, and so
\begin{equation*}
u =
...
\end{equation*}
Now substitute $\xi = x-t$ and $s=t$ to find $u$ in terms of $x$ and $t$:
\begin{equation*}
u
=
e^{-\xi^2-s} + (1 - \xi) e^{-s} +s+\xi-1 
...
\pagebreak[2]
\begin{example}
Finally let's do an example with a variable coefficient
\begin{equation*}
...
$\cos(\xi)$, so that's our initial condition.  So
\begin{equation*}
...
Let us make a few closing remarks.
...
\end{equation*}
This PDE has no solution.  If we had a solution it
...
In the above examples, $b$ was always $1$ meaning we got $s=t$ in the 
characteristic coordinates.  If $b$ is not constant, your expression for $s$
will become more complicated.

We also remark that finding the characteristic coordinates is really
...
Solve
\begin{enumerate}[a)]
\item $u_t +9u_x = 0$, $u(x,0) = \sin(x)$,
\item $u_t -8u_x = 0$, $u(x,0) = \sin(x)$,
\item $u_t +\pi u_x = 0$, $u(x,0) = \sin(x)$.
\end{enumerate}
\end{exercise}
...
{\ }
\begin{enumerate}[a)]
\item Find the characteristic coordinates for the
following equations
\begin{enumerate}[1)]
\item $u_x+u_t + u = 1$, \quad $u(x,0) = \cos(x)$,
\item $2u_x+2u_t +2u = 2$, \quad $u(x,0) = \cos(x)$.
\end{enumerate}
...
\begin{exercise}
Solve $u_t - 5u_x = 0$, $u(x,0) = \frac{1}{1+x^2}$.
\end{exercise}
\exsol{%
$u = \frac{1}{1+{(x+5t)}^2}$
}
