...
To get the error to be less than 0.1, we had to
do 64 steps.  To get it below 0.01, we would have to halve another three or
...
assume that the error goes down by a factor of a half.  Can you estimate the
error in the last line from this?  Does it (approximately) agree with the
...
meaning that if we halve the step size, the error
...
if we halve the step, the error should go down by a factor of
${(\nicefrac{1}{2})}^2 = \nicefrac{1}{4}$.
...
If the set of points where $M$ and $N$ are defined and where $M_y=N_x$
has \myquote{no holes,} such as, for instance, the entire $xy$-plane,
then the equation is exact and a global $F$ can be found.
...
and a potential function existed on that region.
