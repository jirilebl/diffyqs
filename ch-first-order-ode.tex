...
The equation describes the evolution of a function in time.
...
when we encounter the wave equation.
...
the \emph{\myindex{transport equation}}.
...
convection equation: everything travels at some constant speed.
...
\capstart
%mbxSTARTIGNORE
\diffyincludegraphics{width=3in}{width=4.5in}{fopde-transport-1}
\quad
\diffyincludegraphics{width=3in}{width=4.5in}{fopde-transport-2}
%mbxENDIGNORE
%mbxlatex \diffyincludegraphics{width=6in}{width=9in}{sosa-modes-1-2}
\caption{Example of convection for $u_t-u_x = 0$ (that is, $\alpha = 1$) and the
initial condition $f(x)$ is a peak at the origin.  On the left is a graph
of the initial condition $u(x,0)$ and on the right is a graph of
the function $u(x,1)$, that is at time $t=1$, and notice it is one unit to
...
new ODE in the $s$ variable.  In the convection
