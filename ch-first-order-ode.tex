...
\myindex{closed form}\footnote{By closed form, we mean a formula in terms
of known functions with no integrals or limits.}
...
A plot of y equals 1 over quantity 1 minus x,
which is a line starting at a positive value on
the left, then rising towards a vertical asymptote at x equals 1.  Then from
below there is another line that rises from the vertical asymptote
...
The xy plane with a short line segment of slope 3 at the point
(2,1.5).}
...
A grid of line segments whose slope at each point is given by x times y.}
...
A grid of line segments whose slope at each point is given by x times y.
Overlaid are three red lines going through these line segments,
one is concave up above the x-axis, one is simply the x-axis,
and one is concave down below the x-axis.}
...
$y' = -y$.  For the slope field and a few solutions,
...
A grid of line segments with the slope given by minus y.  Overlaid
are three red lines starting at various points on the left-hand side that
all approach the x-axis as we move right.}
...
A grid of line segments with the slope given by 1 over x.  Overlaid
are red lines starting at various points on the left-hand side
and the right-hand side that go out the bottom of the picture
as they approach the y-axis.}
...
A grid of line segments with the slope given by 2 times the
square root of the absolute value of y.  These are lines that
go generally up except on the x-axis where they are horizontal.
There are two lines giving solutions, a horizontal one,
y equals 0, and one
going up from the bottom of the plot and hitting the origin and
then leaving upwards.}
...
\begin{exercise}
Sketch the slope field for $y'=e^{x-y}$.  How do the solutions behave as $x$
grows?  Can you guess a particular solution by looking at the slope
field?
\end{exercise}

\begin{exercise}
Sketch the slope field for $y'=x^2$.
\end{exercise}

\begin{exercise}
Sketch the slope field for $y'=y^2$.
\end{exercise}
...
A slope field where the lines have 0 slope on the y-axis, they are sloped
upwards for negative x and y above some horizontal line and also for
positive x and y below some horizontal line, they are sloped downwards
otherwise.}}
\task
\parbox[c]{1.75in}{\diffyincludegraphics{width=1.75in}{width=1.75in}{yprime1minusxslope}{%
A slope field where the lines are sloped upwards for x less than some
positive quantity and downwards otherwise.  The slope does not seem to vary
with y.}}
\task
\parbox[c]{1.75in}{\diffyincludegraphics{width=1.75in}{width=1.75in}{yprimexminus2yslope}{%
A slope field where above some diagonal line from lower left to upper right,
the slopes are negative, and below this line the slopes are positive.}}
\end{tasks}
\end{exercise}
\end{samepage}
...
%mbxENDIGNORE
%mbxlatex \\
\diffyincludegraphics{width=2in}{width=2in}{yprimey3slope}{A slope field
where the slopes are positive above the x-axis and negative below, getting
steeper and steeper the further away from the x-axis we get.  The slopes are
zero on the x-axis and do not seem to depend on x.}%
%mbxlatex \\
%mbxSTARTIGNORE
}}
%mbxENDIGNORE
...
A slope field where the slopes do not seem to depend on x.  They are
negative at the top of the picture, positive in the middle part, and
negative again at the bottom of the picture.}}
\task
\parbox[c]{1.75in}{\diffyincludegraphics{width=1.75in}{width=1.75in}{yprimecosxyslope}{%
A slope field where the slopes are zero on the x-axis.  Above the x-axis the
slopes first are negative, then turn positive, then turn negative.  Below
the x-axis it is the same pattern but starting with positive, then negative,
then positive again.}}
\task
\parbox[c]{1.75in}{\diffyincludegraphics{width=1.75in}{width=1.75in}{yprimesinxslope}{%
A slope field that is zero on the y-axis.  The slopes do not appear to
depend on y, and from left to right are first positive, then negative then
positive and then negative again.}}
\end{tasks}
\end{exercise}
\end{samepage}
\exsol{%
...
It is not possible for $y'$ to be continuous, $y'(0)=1$ and
...
In \myindex{Leibniz notation}, the equation is
\begin{equation*}
\frac{dy}{dx} = f(x)g(y) ,
\end{equation*}
which we rewrite as
...
assume $y \neq 0$ from now on, so that we can divide by $y$.
...
Plot of the region of the xy-plane near the origin, and
there are two solutions plotted one concave up above the
x-axis and one concave down below the x-axis.}
...
Two plots of the coffee temperature.  On the left, the downwards
solution is shown for time 0 to 12.5 as the temperature goes
from 89 to below 60.  There are horizontal lines at 89, 85,
and 60.  A vertical line at t equals 1 intersects the solution
at temperature 85.  A vertical line at t equals 9.21 intersects the
solution at temperature 60.  The right-hand plot is over longer
period of time to t equals 80.  The solution exponentially decays
towards a horizontal line at temperature 22.}
...
of 22 degrees.  Suppose these temperatures were measured with a precision of
...
are one of the most important types of equations we will learn to solve.
...
\noindent
\inputpdft{lin-tank}{%
A diagram of a tank of liquid with a pipe showing water
flowing into the tank at the top and a pipe showing water leaving
at the bottom.  The top input is labeled as 5 liters per minute,
0.1 kilograms per liter.
The bottom output is labeled as 3 liters per minute.
Inside the liquid is
labeled as 60 liters and 10 kilograms of salt.}
\end{mywrapfigsimp}
A 100-liter tank contains 10 kilograms of salt dissolved in 60 liters of
water.  A solution of water and salt (brine) with a concentration of 0.1
kilograms per
...
for this setup.
Let $x$ denote the kilograms of salt in the tank, and
...
A graph of a solution from t equals 0 to t equals 20
starting at 10 and then rising, at first slowly
then speeding up and ending at roughly 11.86 at the
right-hand side.}
...
As $5$ liters per minute are flowing in and $3$ liters per minute are flowing out,
the volume is increasing by $2$ liters a minute.
So the tank is
full when $60+2t = 100$, or when $t=20$.
We want the value of $x$ when the tank is full,
that is, we compute $x(20)$:
\begin{equation*}
\begin{split}
x(20) & =
\frac{60+40}{10}
...
See \figurevref{linear-salt-graph:fig} for the graph of $x(t)$.
...
%mbxlatex A graph of a solution from t equals 0 to t equals 20
%mbxlatex starting at 10 and then rising, at first slowly
%mbxlatex then speeding up and ending at roughly 11.86 at the
%mbxlatex right-hand side.}
...
In the exercises, feel free to leave the answer as a definite integral if a
...
with a salt concentration of 2 grams per liter is added at a rate
...
Water with
a toxic substance is flowing into the tank at \unitfrac[2]{L}{min},
...
\begin{exercise}
There are bacteria on a plate and a toxic substance is being added that slows
...
1-v' = v^2 .
\end{equation*}
In other words, $v' = 1-v^2$.  We know how to solve such an equation by
...
equation, with the hope of simplifying it.  The table above is just a rule
...
Using substitution, we can also solve the
...
\end{equation*}
Clearly this solution does not work when $x = 0$ (we would, after all,
...
\emph{critical points\index{critical point}} or
\emph{equilibria}\index{equilibrium}.
...
A plot for t from 0 to 20 and x from minus 10 to 10 with
a grid of lines with slopes of 0.3 times quantity 5 minus x.  These
slopes are going up below the x equals 5 line and are going down
above the x equals 5 line.  The slopes are steeper the further away
from x equals 5 line we are.  Several solutions are shown which
are lines starting at various points on the left and then
all converging towards the x equals 5 line as t increases.}
...
A plot for t from 0 to 20 and x from minus 5 to 10 with
a grid of lines with slopes of 0.1 times x times quantity 5 minus x.
These slopes are going down below the x equals 0 line
and above the x equals 5 line.
Between x equals 0 and x equals 5 the slopes are going up.
Several solutions are shown which when starting below x equals 0 just keep
going down and leave the plot.  When the solution is above x equals 0,
then it rises and then approaches the x equals 5 line.  When the solution
starts above x equals 5, it goes down towards and approaches
the x equals 5 line.
The lines x equals 0 and x equals 5 are also solutions.}
 \caption{The slope field and some solutions of
...
less catastrophic
predictions about world population than $x'=kx$.  In the real world, there is no
...
unstable.
It is not necessary to find the exact solutions to understand their
long-term behavior, that is, behavior as time goes to infinity.
From the slope field above of
$x' = 0.1 x(5-x)$, we see that
\begin{equation*}
\lim_{t\to \infty} x(t) =
\begin{cases}
5 & \text{if } \; x(0) > 0 , \\
...
Think of the equation $x' = x^2$.
We have seen that solutions only exist for some finite period of time.
Same can happen
...
a down arrow.
To figure out the sign, we could just plug in some $x$ between the critical
points because $f(x)$ will have the same sign at all $x$ between two critical
points as long as $f(x)$ is continuous.
...
A vertical line with two points marked with horizontal ticks
marked as x equals 0 for the lower one and x equals 5 on the upper one.
There are triangular arrows on the line.
Below x equals 0 the arrow points down,
between the x equals 0 and x equals 5 ticks the arrow points up,
and above the x equals 5 tick
...
On the left, there are three possible phase diagrams with one
critical point marked as a horizontal tick.
The first one has the arrow above
and below the tick pointing away from the tick.  The second one has
both arrows pointing up, the third has both arrows pointing down.
These three diagrams are labeled as 'unstable'.  Then there is one
phase diagram with one critical point on the right where both
arrows are pointing towards the critical point.
This diagram is labeled as 'stable'.}
\end{center}
...
For that graph, note that $x$ is the independent variable and will
...
A plot for t from 0 to 20 and x from 0 to 10 with
a grid of lines with slopes of
0.1 times x times left parenthesis 8 minus x right parenthesis minus 1.
These slopes are going down below roughly x equals 1.55 and above roughly
x equals 6.45.  Between these two lines the slopes are going up.
Several solutions are shown, which when starting below x equals 1.55 keep
going down and leave the plot.  When the solution starts above
x equals 1.55, then it rises and then approaches the line at roughly
x equals 6.45.
When the solution starts above x equals 6.45, it goes down towards and
approaches this line at roughly x equals 6.45.  The two horizontal lines
are also solutions.}
 \caption{The slope field and some solutions of
...
A plot for t from 0 to 20 and x from 0 to 10 with
a grid of lines with slopes of
0.1 times x times left parenthesis 8 minus x right parenthesis minus 1.6.
In this
plot there is only one horizontal line at x equals 4.  The slopes are all
going down above and below this line, more steeply the further away
from the line we are.  Three solutions are drawn, one is the
equilibrium solution as a horizontal line x equals 4. Another solution
going down from above and approaching the x equals 4 is drawn and a solution
starting slightly below the x equals 4 line and then going down and
dropping off the picture before we get to the right-hand side.}
 \caption{The slope field and some solutions of
...
4 million.  However, if it ever drops below 4 million, perhaps due to a worse than
...
A plot for t from 0 to 20 and x from 0 to 10 with
a grid of lines with slopes of
0.1 times x times left parenthesis 8 minus x right parenthesis minus 2.
In this plot, there are no horizontal lines and all the slopes are down.
The slopes are steeper at the top of the picture, they get slightly less
steep as we get to the middle of picture and then get steeper again at the
bottom.  Three solutions are drawn, all going down.
Solutions start steeply down at the
top of the picture then go down a bit more slowly in the middle part and
then become steep again at the bottom.}
\caption{The slope field and some solutions of
...
$\frac{dx}{dt} = kx(M-x)$.
Suppose we modify our harvesting.  We will only harvest
an amount proportional to the current population.  In other words, we harvest $hx$
per unit of time
...
We call $h$ the \emph{\myindex{step size}}.
...
Two plots in the xy-plane of a slope field with slopes y squared over 3
in the region where x is between minus 1 and 3 and y is between 0 and 3.
All the slopes are upwards with the ones further up being steeper.
In the left plot, the first step of Euler's method is shown as
a green line starting at (0,1) of slope 1 over 3, which is the slope at
(0,1) and going right until x equals 1.  In the right plot, a second
line is added starting at the end of the first line with slope equal
to the slope of the slope field and going further right until x equals 2.}
\caption{First two steps of Euler's method with $h=1$ for
the equation $y' = \frac{y^2}{3}$ with initial condition $y(0)=1$.%
...
A plot in the xy-plane of a slope field with slopes y squared over 3
in the region where x is between minus 1 and 3 and y is between 0 and 3.
The first two steps are shown the two green line segments as before,
but also the solution through (0,1) is drawn in red, which is curved
upwards and follows the slope field.  We can see that it stays above the
green line segments.}
...
\end{tabular}
\end{center}
\caption{Euler's method approximation of $y(2)$ for
...
\end{tabular}
\end{center}
\caption{Attempts to use Euler's to approximate $y(3)$ for
...
A plot in the xy-plane for x between roughly minus 13 to 13 and y
between minus 10 to 10.  Several circles centered at the origin, (0,0),
are drawn.}
...
these curves are sometimes called \emph{\myindex{equipotential curves}}.}:
...
For example,
in \exampleref{exact:example:2xyandxy},
...
\begin{theorem}[Poincar\'e Lemma]\index{Poincar\'e Lemma}
...
M_y = 1 = N_x .
\end{equation*}
The Poincar\'e Lemma applies, so let us look for $F$.
...
See \figurevref{exact:y2x}, and note again that the solution only exists for
$x > 0$.
\begin{myfig}
\capstart
\diffyincludegraphics{width=3in}{width=4.5in}{exact-y2x}{%
A plot in the xy-plane for x between minus 5 to 5 and y from minus 10 to 10
is shown.  A blue line is drawn starting at (0,0) to the top right
corner at (5,10) with a red dot marking the initial point
drawn at (1,2).}
\caption{Solution to
...
has \myquote{no holes,} such as, for instance, the entire $xy$-plane,
then the equation is exact and an $F$ defined on that region can be found.
For example, in \exampleref{exact:exactR2example},
$M=2x+y$ and $N=x-1$ are defined in the entire plane and
$M_y = 1 = N_x$, so a global $F$ exists there.
Similarly, in \exampleref{exact:examplevortex}
the half-plane where $x > 0$ also has \myquote{no holes}
and a potential function exists on that region.
So in such cases, checking
$M_y = N_x$ is good enough to check for exactness.
...
But how do we find the integrating factor $u$?  Well, for
...
\begin{tasks}
\task
Show that if $u$ is harmonic, $-u_y \, dx + u_x \, dy = 0$ is an exact
...
A diagram of the xt-plane with three lines drawn.  All three
line start on the x-axis and are going steeply up and to the right.
The one starting at the origin is marked as xi equals 0,
the one to the right
is marked as xi equals 1,
and then the third one to the right of that is marked
xi equals 2.}
...
the position along the river, $t$ is the time, and $u(x,t)$ the
...
Two plots in the plane for x between minus 3 to 3 and the dependent
axis between a bit less than 0.0 and a bit above 1.0.  On the left plot,
there is a graph of a function which is mostly horizontal at level 0
except for a sharp peak up to level 1 when x equals 0.
On the right plot, it is the same exact peak but it is now centered
at x equals 1.}
...
A diagram of the xt-plane with a sample characteristic curve
drawn starting on the x-axis and going up.  The point on the curve that
sits on the x-axis is marked as x equals xi and also s equals 0.
The general point
on the curve is marked as (x of s, t of s).
The entire curve is marked as
xi equals constant.}
...
A surface plot of a function u of (x,t) for x between minus 3 and 3 and t
between 0 and 3.  When t equals 0, the surface looks like a
flat line then a peak and going back down towards a flat line.
As t grows the surface becomes, as t grows,
closer and closer to a sideways
flat surface that goes up as x grows.
At t equals 3 it looks almost completely
straight from a bit less than minus 3 at x equals minus 3
and goes up to a bit below 2.}
...
general, if $a$ and $b$ are not linear functions or constants, finding
closed-form expressions for the characteristic coordinates may be impossible.
