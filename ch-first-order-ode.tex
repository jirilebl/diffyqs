...
We might sometimes get stuck even if we can do the
...
The above equation also has the solution $y=0$.
...
Solve
\begin{tasks}(2)
\task $u_t +9u_x = 0$, $u(x,0) = \sin(x)$,
\task $u_t -8u_x = 0$, $u(x,0) = \sin(x)$,
\task $u_t +\pi u_x = 0$, $u(x,0) = \sin(x)$.
\end{tasks}
\end{exercise}
