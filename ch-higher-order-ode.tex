...
along the circular arc away from the center labeled m L theta'', a
force arrow in the opposite direction (towards the center) labeled m g
sin(theta), and an arrow directly downwards from the mass labeled m g.
...
A plot of sin(theta) and theta, for theta between -1 and 1.  The plot of
theta is simply a straight line from (-1,-1) to (1,1), while the plot of
sin(theta) is almost the same but it starts a little bit above the straight
...
goes between roughly -0.707 and 0.707, starting at (0,0.5) going upwards,
...
and one starting at (0,-1) are shown.  A sinusoidal-like curve winds between
...
%mbxlatex and one starting at (0,-1) are shown.  A sinusoidal-like curve winds between
...
A plot of three versions of a function C(omega) for omega between 0 and 3.
The line on top starts at 1, rises up to a peak of a little more than 2.5
just before omega=1, then it decays down towards zero.  The middle line does
...
the same small value at omega=3.}
