...
The answer is affirmative!  Provided that $y_1$ and $y_2$ are different enough in
...
In some sense, a double root rarely happens.  If coefficients are 
picked randomly, a double root is unlikely.
There are, however, some real-world problems
where a double root does happen naturally (e.g.\ critically damped
...
root of $-1$.  We follow the mathematicians' convention and use $i$.
...
solve the equation with some constants $c_1,c_2,\ldots,c_n$,
where for example $c_1 \neq 0$, then we
...
polynomial such as this is the product of the negations of all the roots
...
This equation appears in lots of applications even if it does not
seem like it at first.  Many real-world scenarios can be simplified to
a mass on a spring.  For example, a bungee jump is essentially a mass
...
\noindent
\inputpdft{mv-rlc}{%
A diagram of a circuit.  Following the diagram counterclockwise, we start on
the left with an electric source labeled E, then a resistor on the bottom
labeled R, then an inductor on the right labeled L, then a capacitor on the
top labeled C, then we connect back to the electric source.}
\end{mywrapfigsimp}
...
The length of the pendulum is labeled L.  There is a force arrow on m
along the circular arc away from the center labeled
m times L times second derivative of theta, a
force arrow in the opposite direction (towards the center) labeled
m times g times sin of theta,
and an arrow directly downwards from the mass labeled m times g.
The latter two arrows form a right triangle with the downward arrow forming
the hypotenuse.}
\end{mywrapfigsimp}
Our next example behaves like a mass and spring system only
approximately. Suppose a
mass of $m$ kilograms hangs on a pendulum of length $L$ meters.  We seek an equation for
the angle $\theta(t)$ (in radians).  Let $g$ be the acceleration due to
gravity in meters per second squared.
Let us derive an equation for $\theta$ using \myindex{Newton's second law}:
...
The $m$ curiously cancels from the equation and we end up with the equation
\begin{equation*}
...
A plot of sin of theta and also of just theta,
for theta between minus 1 and 1.
The plot of
theta is simply a straight line from (minus 1,minus 1) to (1,1),
while the plot of
sin of theta is almost the same but it starts a little bit above the straight
line on the left and ends a little bit below on the right.}
...
A graph of the function for t from 0 to 10, which is a sinusoidal curve that
goes between roughly minus 0.707 and 0.707,
starting at (0,0.5) going upwards,
making a little bit more than 3 cycles in the picture.}
...
A plot of 3 functions for t between 0 and 100.  All three graphs start at
(0,1) and they all approach the horizontal axis being almost
indistinguishable from it by the right side of the picture.  The first one
starts downwards, crosses the axis and then goes back up and approaches it,
the second starts out horizontally but turns downwards and approaches the
axis, and the third starts upwards, then turns downwards quickly and
approaches the axis.}
...
A plot for t between 0 and 30.  Two decaying exponential curves one starting at (0,1)
and one starting at (0, minus 1) are shown.  A sinusoidal-like curve winds between
them, starting at the top curve, going down the bottom curve, then back up
and so on.}
...
does $k$ have to be so that the mass does not go further than 3 meters
...
Also make sure to look for duplication in the entire expression you try.  For
...
$y_1'' = - y_1$
and $y_2'' = - y_2$.
(If the equation were the more general
...
A plot of a quickly oscillating curve whose amplitude starts at zero, then
grows, then goes to zero, then grows again, etc.}
\caption{Graph of
...
in \figurevref{3.6:beatingfig}.  To analyze it, we
use the 
...
A quickly oscillating curve whose amplitude starts at zero but then
grows linearly as we move to the right in the plot.}
\caption{Graph of
...
$x_p$ the \emph{\myindex{steady periodic solution}} and denote it
by $x_{sp}$.
The general solution is
\begin{equation*}
...
A plot of several solutions that start at different points on the left-hand
side.  As t goes from 0 to 20, they all approach and are almost
indistinguishable from an oscillating sinusoidal curve.}
...
Conversely, the smaller the damping, the longer the \myquote{transient region.}
This is consistent
...
is the \emph{\myindex{practical resonance frequency}}.
...
\figurevref{3.6:pracresfig} shows a
...
A plot of three versions of a function C of omega for omega between 0 and 3.
The line on top starts at 1, rises up to a peak of a little more than 2.5
just before omega equals 1, then it decays down towards zero.  The middle line does
roughly the same thing but reaches a smaller peak a little earlier.  The
bottom line simply goes downwards reaching no peak.  They all have roughly
the same small value at omega equals 3.}
