...
\end{equation*}
Assuming a solution of a differential equation is a
power series, we can perhaps use a method reminiscent of undetermined
coefficients---we try to solve for the numbers $a_k$.
...
The $a_k$ are called the \emph{coefficients}%
\index{coefficients of power series}%
\index{power series coefficients} and $x_0$ is called
the \emph{center}\index{center of a power series}%
\index{power series center}.
Let
\begin{equation*}
...
\end{equation*}
converges at every $x$.
Here, $a_k = \nicefrac{1}{k!}$ and $x_0 = 0$.
...
no further than $x_1$ to $x_0$), we have
$\lvert a_k \rvert \, {\lvert x-x_0 \rvert}^k \leq
\lvert a_k \rvert \, { \lvert x_1-x_0 \rvert}^k$
for all $k$.
As the numbers $\lvert a_k \rvert \, {\lvert x_1-x_0 \rvert}^k$
sum to some finite
limit, summing smaller positive numbers
$\lvert a_k \rvert \, {\lvert x-x_0 \rvert}^k$
must also have a finite limit.
...
\capstart
\inputpdft{ps-conv}{%
A diagram of the number line with the points x sub 0 minus rho,
x sub 0, and x sub 0 plus rho marked.
The segment between x sub 0 minus rho and x sub 0 plus rho is shaded and marked with
'converges absolutely'.  The parts of the line below x sub 0 minus rho and
above x sub 0 plus rho is labeled 'diverges'.}
...
We apply this test to the power series \eqref{ps:sereq1}. 
...
The \emph{root test} is similar\index{root test for series}.
Suppose
\begin{equation*}
...
Moreover, if either limit for 
$A$ diverges to $\infty$, then the series is divergent.
\pagebreak[3]
\end{theorem}
...
$\sqrt[k]{\lvert a_k \rvert}$
might not exist.
Sometimes the test must be applied to the series itself---to find
the $L$ rather than the $A$.  For example, for the series
$\sum_{k=1}^\infty 2^k x^{2k}$, we cannot apply the root with the $A$, we
must work with the limit of $\sqrt[k]{\lvert 2^k x^{2k} \rvert}$,
which equals $L = 2 {\lvert x \rvert}^2$.  This $L$ is less than $1$ when
$\lvert x \rvert < \frac{1}{\sqrt{2}}$ and so $\frac{1}{\sqrt{2}}$
is the radius of convergence.
Other ways exist to find the radius, but ratio and root tests
cover many series arising in practice.
...
%cover many of the series that arise in practice.
%Often both tests apply, though the limit might
%be easier to compute in one test than the other.

At the endpoints,
$x = x_0-\rho$ and $x = x_0+\rho$,
the series may or may not converge, and the tests above
say nothing about convergence there.
Sometimes convergence at the endpoints is important, but for
our purposes, we will not worry about it much.

\subsection{Analytic functions}

Functions represented by power series are called
\emph{\myindex{analytic functions}}.  Not all functions are analytic,
but the functions you have seen in calculus likely all are.
An analytic function $f(x)$ equals its \emph{\myindex{Taylor series}}%
...
A plot of the sine function on the interval minus 10 to 10.
Then two approximations plotted that both come from the bottom of the
picture approach the sine function graph, both are very close to it
in an interval around the origin, and both leave after a while through
the top of the picture.  One of the approximations is close to the
graph of the sine for a longer interval.}
...
Instead of applying
the Taylor series expansion \eqref{ps:tayloreq},
the geometric series together with rules for addition and
multiplication of power series can be used to expand any rational function around
a point $x_0$, as long as the denominator is not zero at $x_0$.
...
to use the method of partial fractions.  For example,
...
\begin{exercise}
Is $\displaystyle \sum_{k=0}^\infty e^k x^k$ a convergent power series?
If so, find the radius of convergence.
\end{exercise}

\begin{exercise}
Is $\displaystyle \sum_{k=0}^\infty k x^k$ a convergent power series?
If so, find the radius of convergence.
\end{exercise}
...
\begin{exercise}
Is $\displaystyle \sum_{k=0}^\infty \frac{1}{(2k)!} {(x-10)}^k$
a convergent power series?  If so, find the radius of convergence.
\end{exercise}
...
\begin{exercise}
Find the Taylor series for $\ln x$ around the point $x_0 = 1$,
...
\begin{exercise}
Reindex the following series to have the powers be $x^k$.
\begin{tasks}(3)
...
\end{equation*}
and solve for the $a_k$ to obtain a solution defined in some
interval around $x_0$.
...
\end{equation*}
are defined for $x$ near $x_0$.  If $P(x_0) = 0$, then $x_0$
is a \emph{\myindex{singular point}}.  Handling singular points is
harder than handling ordinary points and so we now focus only on ordinary points.
...
\end{equation*}
We need to subtract $y$ from $y''$, so 
we reindex the series for $y''$ (replace $k$ with $k+2$):
\begin{equation*}
...
\end{split}
\end{equation*}
We reindex and synchronize the start indexes to make the series
possible to sum:
\begin{equation*}
\begin{split}
0 = y''-xy
...
\end{equation*}
We have arbitrary constants $a_0$ and $a_1$, and $a_2 = 0$.
We compute a few higher coefficients:
\begin{multline*}
...
\end{equation*}
Finally, since $a_2 = 0$, we have $a_5 = 0$, $a_8 = 0$, $a_{11}=0$, etc.
...
$y(x)= a_0 y_1(x) + a_1 y_2(x)$.  If we plug $x=0$ into the
...
Graph of two functions, they both seem to be oscillating up and down
for x less than 0, and both seem to go up like an exponential for
positive x.  One of them goes through the origin with slope 1, one of
them goes through the point (0,1) with slope zero.}
...
\sectionnotes{1.5 or 2 lectures\EPref{, \S8.3 and \S8.4 in \cite{EP}}\BDref{,
...
P(x) y'' + Q(x) y' + R(x) y = 0
\end{equation*}
be an ODE for polynomials
$P(x)$, $Q(x)$, and $R(x)$.
As before, if $P(x_0) = 0$, then $x_0$ is a
singular point.
...
If $x_0 \neq 0$, then in the solution, we would replace every $x$ with $(x-x_0)$.
The main
...
\end{equation*}
We plug this $y$ into the left-hand side of
equation \eqref{eq:frobeniusmethod}, collect
terms, and write everything as a single series.
\item
The obtained series must be zero.  Setting the first
coefficient (the one with the smallest power) in the series
...
an integer, then, assuming $r_1$ is the larger root, one solution is
\begin{equation*}
y_1 = x^{r_1} \sum_{k=0}^\infty a_k x^{k} ,
\end{equation*}
...
\end{equation*}
results in a complex-valued function---the $a_k$ are complex
...
\end{equation*}
Unless $p=0$, we obtain two roots, $r_1 = p$ and $r_2 = -p$.
...
to find the solutions $y_1$ and $y_2$ above.
\end{tasks}
\end{exercise}

Bessel functions are convenient constant multiples of $y_1$ and $y_2$.
First, we define the \emph{gamma function}
\begin{equation*}
...
\Gamma(x+1) = x \Gamma(x) .
\end{equation*}
From this property, it follows that $\Gamma(n) = (n-1)!$ when $n$ is a
positive integer.  So the gamma function is a continuous version of the factorial.  We
compute:
\begin{align*}
...
we define the gamma function for 
all $x$ except nonpositive integers. 
The reciprocal $\frac{1}{\Gamma(x)}$
can be defined for all $x$ by letting $\frac{1}{\Gamma(x)}$ be zero
for integers $x \leq 0$.
...
for convenience.
As the reciprocal of $\Gamma$ is defined for all $x$, the formulas
on the right hold even for integer $p$.
...
and $J_{-p}$ are linearly independent.
For integer orders, one can check that
\begin{equation*}
J_n(x) = {(-1)}^nJ_{-n}(x) .
\end{equation*}
So $J_{-n}$ is not a second linearly independent
...
Two plots are shown.  In the left-hand plot, two functions that seem
to oscillate are graphed for x from 0 to 10, with one starting at
0 but with positive slope,
and one starting at 1 with slope zero.  Their amplitude
seems to be decreasing as x gets larger.  In the right-hand plot,
similarly two similar plots are shown but now both of them seem to start
from minus infinity and go up along the y axis first.}
