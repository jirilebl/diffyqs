...
Another very similar test is the \emph{\myindex{root test}}.  Suppose
\begin{equation*}
L = \lim_{k\to\infty} \sqrt[k]{\lvert c_k \rvert}
\end{equation*}
exists.  Then $\sum_{k=0}^\infty c_k$ converges absolutely if $L < 1$
and diverges if $L > 1$.  For power series we find
...
be easier to compute in one way that the other.
...
%mbxENDIGNORE
%mbxlatex \diffyincludegraphics{width=6in}{width=9in}{bessel-first-second}
